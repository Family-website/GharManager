...
// ==========================================
// ☁️ CLOUD DATA & GLOBALS 
// ==========================================
let familyExpenses = []; let dudhRecords = []; let rationItems = []; let investments = []; let activeLoans = []; let activeSubs = []; let rechargeRecords = [];
...
async function saveToCloud() { 
    ...
    localStorage.setItem('familyExpenses', JSON.stringify(familyExpenses)); localStorage.setItem('rechargeRecords', JSON.stringify(rechargeRecords));
    if(isOffline) return Swal.fire({toast:true, position:'top-end', icon:'info', title:'Saved Offline 💾', timer:2000, showConfirmButton:false});
    try {
...
async function syncOldLocalData() { 
    ...
    const localExp = JSON.parse(localStorage.getItem('familyExpenses')); const localRech = JSON.parse(localStorage.getItem('rechargeRecords'));
    if (localExp && localExp.length > 0 && familyExpenses.length === 0) { familyExpenses = localExp; dataChanged = true; } 
    if (localRech && localRech.length > 0 && rechargeRecords.length === 0) { rechargeRecords = localRech; dataChanged = true; }
    if (dataChanged) { await saveToCloud(); localStorage.removeItem('familyExpenses'); localStorage.removeItem('rechargeRecords'); } 
}
...
function checkStreak() {
    ...
    if(lastLoginDate !== today) {
        ...
        if(dailyStreak > 1) { Swal.fire({ title: '🔥 Streak Maintained!', text: `${dailyStreak} days!`, icon: 'success', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false }); gainXP(5); }
    }
...
function openProfile() {
    ...
    } catch (err) { console.error("Profile Error: ", err); Swal.fire('Minor Glitch', 'Kuch data load hone me issue aaya!', 'info'); }
}
...
function renderCalendar(exps, fm) {
    ...
    for(let i=1; i<=dim; i++) {
        ...
        if(dt[i]) { el.style.color = 'white'; if(dt[i] > 2000) el.style.background = '#7f1d1d'; else if(dt[i] > 1000) el.style.background = '#ef4444'; else if(dt[i] > 300) el.style.background = '#f59e0b'; else el.style.background = '#10b981'; } else { el.style.background = 'var(--line-color)'; el.style.color = 'var(--text-main)'; }
        frag.appendChild(el);
    }
...
// 🔥 THE MEGA OPTIMIZATION: USING DOCUMENT FRAGMENT FOR 10X SPEED
function updateHisabUI() {
    ...
    let fragment = document.createDocumentFragment(); // ⚡ DocumentFragment for ultra-fast rendering
    ...
    list.appendChild(fragment); // Render everything in 1 microsecond!
    ...
    let prediction = (totalExpense / new Date().getDate()) * new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
    let pEl = document.getElementById('predicted-expense'); if(pEl) pEl.innerText = `🔮 AI: ₹${Math.round(prediction||0)}`;
    
    renderCalendar(filteredExpenses, filterMonth); applyLanguageUI();
}

// 🛡️ BUG FIX: ONLY CALL CHARTS IN PROFILE TO AVOID CRASHES
...
// ==========================================
// 💸 RECHARGE, INVESTMENTS & OTHERS
// ==========================================
function addRecharge() { const name = document.getElementById('rech-name').value; const amt = parseFloat(document.getElementById('rech-amt').value); const days = parseInt(document.getElementById('rech-days').value); const dDate = document.getElementById('rech-date').value || todayDateString; if(!name || isNaN(amt) || isNaN(days) || amt<=0 || days<=0) return Swal.fire('Error', 'Sahi details daalein!', 'error'); let startDate = new Date(dDate); startDate.setDate(startDate.getDate() + days); let expDate = startDate.toISOString().split('T')[0]; rechargeRecords.push({ name: name, amount: amt, days: days, date: dDate, expiry: expDate }); familyExpenses.push({ member: "Aditya", category: "Bills", description: `📱 Recharge: ${name}`, amount: amt, date: dDate, receipt: "", gps: null }); saveToCloud(); updateRechargeUI(); updateHisabUI(); document.getElementById('rech-name').value = ''; document.getElementById('rech-amt').value = ''; document.getElementById('rech-days').value = ''; playSound('success'); Swal.fire('Saved!', `Recharge Hisaab me add ho gaya. Expire hoga: ${expDate}`, 'success'); }
function updateRechargeUI() { const list = document.getElementById('recharge-list'); if(!list) return; list.innerHTML = ''; if(rechargeRecords.length === 0) list.innerHTML = `<div style="text-align:center; padding: 20px; opacity:0.6;"><h3 style="color:var(--text-main); font-size:14px;">No Recharges 📱</h3></div>`; let today = new Date(); rechargeRecords.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach((rech, index) => { let expD = new Date(rech.expiry); let diffTime = expD - today; let diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); let statusHtml = ""; let borderCol = ""; if(diffDays < 0) { statusHtml = `<span style="color:#ef4444; font-weight:bold; font-size:11px;">Expired</span>`; borderCol = "#ef4444"; } else if(diffDays <= 3) { statusHtml = `<span style="color:#f59e0b; font-weight:bold; font-size:11px;">Expiring in ${diffDays} days ⚠️</span>`; borderCol = "#f59e0b"; } else { statusHtml = `<span style="color:#10b981; font-weight:bold; font-size:11px;">Active (${diffDays} days left) ✅</span>`; borderCol = "#10b981"; } const li = document.createElement('li'); li.style.borderLeft = `4px solid ${borderCol}`; li.innerHTML = `<div class="list-left"><strong style="font-size:16px;">${rech.name}</strong><span style="font-size:11px; color:#64748b;">Done: ${rech.date} | Valid: ${rech.days}D</span><br>${statusHtml}</div><div class="list-right"><span style="font-weight:800; color:#2563eb; font-size:18px; margin-right:5px;">₹${rech.amount}</span><button class="action-btn delete" onclick="deleteRecharge(${index})">🗑️</button></div>`; list.appendChild(li); }); }
...
// ==========================================
// 🤖 11. AI FINANCE TIPS & BOTTOM SHEET
// ==========================================
