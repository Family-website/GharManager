...
async function sendToRealAI() {
    ...
    chatBox.innerHTML += `<div style="margin:5px 0; text-align:right;"><span style="background:#3b82f6; color:white; padding:8px 12px; border-radius:16px 16px 0 16px; display:inline-block; max-width:80%; word-wrap:break-word; font-size:14px;">${userText}</span></div>`;
    ...
    Always answer in simple Roman Hindi (Hinglish). Do not use special characters or heavy formatting.
    User Query: ${userText}`;

    let loadingId = 'load-' + Date.now();
    chatBox.innerHTML += `<div id="${loadingId}" style="margin:5px 0; text-align:left;"><span style="background:var(--paper-bg); color:var(--text-main); border:1px solid #cbd5e1; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; font-size:14px;">AI is thinking... ⏳</span></div>`;
    ...
    try {
        ...
        if (data.candidates && data.candidates[0].content) {
            ...
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left; animation: fadeIn 0.3s;"><span style="background:var(--paper-bg); color:var(--text-main); border:1px solid #cbd5e1; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; max-width:90%; word-wrap:break-word; line-height:1.4; font-size:14px;">${formatText}</span></div>`;
        } else {
...
function openAIBottomSheet() {
    Swal.fire({
        title: '🤖 AI Assistant',
        // 🔥 FIX: Input Box is Back & Beautiful!
        html: `<div style="display:flex; flex-direction:column; gap:10px;">
                ...
                <p style="font-size:12px; font-weight:bold; color:var(--ink-blue); text-align:left; margin:0;">💬 Chat with AI Master</p>
                <div id="ai-chat-box" style="background:var(--line-color); height:220px; border-radius:12px; padding:10px; font-size:14px; overflow-y:auto; text-align:left; margin-bottom:5px; scroll-behavior: smooth;">
