...
window.addEventListener('load', () => {
    ...
    // Fallback: hide splash after max 3s even if Firebase is slow
    const maxSplashTimer = setTimeout(() => hideSplash(), 3000);
    window._maxSplashTimer = maxSplashTimer;

    if(!navigator.onLine) {
        // Offline: hide splash quickly after bar animation
...
function hideSplash() {
    ...
    if(window._maxSplashTimer) { clearTimeout(window._maxSplashTimer); window._maxSplashTimer = null; }
    ...
    if(!localStorage.getItem('onboarding_done')) { const os = document.getElementById('onboarding-screen'); if(os) os.style.display = 'flex'; }
    else if(!currentUser) { const ls = document.getElementById('login-screen'); if(ls) ls.style.display = 'flex'; }
    updateHisabUI();
}
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        // Hide splash as soon as auth is confirmed — no artificial wait
        hideSplash();
        ...
        // No current user — hide splash after short animation completes
        hideSplash();
    }
});
...
function updateHisabUI() {
    ...
    if(alertBanner) {
        ...
        if(pct >= 100) {
            ...
            alertBanner.className = 'danger';
            alertBanner.innerHTML = `🚨 Budget Khatam Ho Gaya! ₹${Math.round(totalExpense - budgetLimit)} zyada kharcha.`;
        } else if(pct >= 80) {
            alertBanner.style.display = 'block';
            alertBanner.className = '';
            alertBanner.innerHTML = `⚠️ Budget ka ${Math.round(pct)}% kharach ho gaya! Sirf ₹${Math.round(budgetLimit - totalExpense)} bacha hai.`;
        } else {
...
// ── HOME TREND CHART (7-day mini bar chart on dashboard) ──────────
let homeTrendChartInstance = null;
function renderHomeTrendChart() {
    const wrap = document.getElementById('home-trend-wrap');
    ...
    const dayTotals = last7.map(dateStr => {
    ...
    const labels = last7.map(d => {
        ...
        return dt.toLocaleDateString('hi-IN', { weekday: 'short' });
    });

    const maxSpend = Math.max(...dayTotals);
    const colors = dayTotals.map(v => {
        if(!v) return 'rgba(148,163,184,0.25)';
        if(v >= maxSpend * 0.8) return '#ef4444';
        if(v >= maxSpend * 0.5) return '#f59e0b';
        return '#10b981';
    });

    const tc = document.documentElement.getAttribute('data-theme') === 'night' || document.documentElement.getAttribute('data-theme') === 'cyberpunk' ? '#7c91b0' : '#64748b';

    if(homeTrendChartInstance) homeTrendChartInstance.destroy();
    homeTrendChartInstance = new Chart(ctx.getContext('2d'), {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
        ...
        options: {
            ...
            scales: {
                x: { ticks: { color: tc, font: { size: 11, weight: '700' } }, grid: { display: false } },
                y: { ticks: { color: tc, font: { size: 10 }, callback: v => '₹' + Math.round(v) }, grid: { color: 'rgba(148,163,184,0.1)' } }
            }
        }
    });
}
