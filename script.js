...
function updateHisabUI() {
    ...
    if(filteredExpenses.length === 0) {
        ...
        filteredExpenses.forEach((item) => {
            ...
            // 🗑️ BUTTON REMOVED (Share, QR, Edit) - ONLY DELETE BUTTON REMAINS
...
// ==========================================
// 🖨️ BUG FIX 1: PDF TEXT ENCODING & EXPORT
// ==========================================
...
function startVoice() { 
    ...
    try {
        ...
        recognition.onresult = (event) => { 
            ...
            if(match) { 
                let amt = match[0];
                let descStr = text.replace(match[0], '').replace(/(rupaye|rupay|roopaye|rs|ka|ki|ke)/gi, '').trim(); 
                if(!descStr) descStr = "Voice Kharcha";
                ...
                setTimeout(() => {
                    addExpense();
                    Swal.fire({ toast:true, position:'top-end', icon:'success', title:`✨ AI ne ${descStr} (₹${amt}) add kar diya!`, timer: 2500, showConfirmButton: false });
                }, 200);
            } 
            else { 
                document.getElementById('description').value = text; 
                document.getElementById('description').dispatchEvent(new Event('input'));
                Swal.fire({ toast:true, position:'top-end', icon:'info', title:'Amount nahi mila, manually add karein.', timer: 2000, showConfirmButton: false });
            }
        ...
        recognition.onerror = (e) => { 
            ...
            if(e.error === 'not-allowed') {
                Swal.fire('Permission Denied', 'Browser me Microphone ki permission on karein!', 'error');
            } else {
                Swal.fire('Error', 'Awaz clear nahi aayi!', 'error'); 
            }
        }; 
        ...
        console.error(e); 
        Swal.fire('Error', 'Mic start nahi hua. ' + e.message, 'error'); 
    }
}
