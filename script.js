...
// Phase 2 fix: explicitly set session persistence instead of relying on
// Firebase's implicit default. LOCAL = user stays logged in across
// browser restarts/tabs, same behaviour the app already had, but now
// it's a deliberate, documented choice instead of an assumption.
auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL).catch((e) => {
    console.warn('Auth persistence could not be set:', e);
});

// Phase 6.2 fix: enable Firestore's own offline persistence instead of
// building a custom Background Sync + Service Worker bridge. This is
// the more robust option — Firestore already queues writes made while
// offline in IndexedDB and replays them automatically the moment the
// connection returns, and it survives page reloads/app restarts. A
// hand-rolled Background Sync handler would have had to reimplement
// this same queuing logic (and the Firestore compat SDK isn't easily
// usable from inside a Service Worker anyway, since sw.js runs in a
// separate worker context without DOM/page access).
// synchronizeTabs lets multiple open tabs share one offline cache
// instead of fighting over it.
db.enablePersistence({ synchronizeTabs: true }).catch((err) => {
    if (err.code === 'failed-precondition') {
        console.warn('Offline persistence: multiple tabs open without sync support — using memory cache instead.');
    } else if (err.code === 'unimplemented') {
        console.warn('Offline persistence: this browser doesn\'t support it — app still works, just without offline write queuing.');
    } else {
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        // Phase 2 fix: gentle, dismissible reminder for unverified
        // email/password accounts. Google sign-ins are always
        // pre-verified so they're excluded. This never blocks access —
        // existing users who never verified keep working normally.
        const usesPasswordAuth = user.providerData && user.providerData.some(p => p.providerId === 'password');
        if (usesPasswordAuth && !user.emailVerified) {
            setTimeout(() => {
                Swal.fire({
                    toast: true, position: 'top', icon: 'info',
                    title: 'Apna email verify kar lo — inbox mein link bheja tha 📧',
...
function registerWithEmail() { const email = document.getElementById('email-input').value.trim(); const password = document.getElementById('password-input').value.trim(); if (!email || password.length < 6) return Swal.fire('Oops!', 'Details daalein.', 'warning'); const btn = document.getElementById('main-login-btn'); if(btn){btn.innerHTML='⏳ Creating...';btn.disabled=true;} auth.createUserWithEmailAndPassword(email, password).then((cred) => { if (cred && cred.user) { cred.user.sendEmailVerification().catch((e) => console.warn('Verification email failed:', e)); } Swal.fire('Mubarak ho! 🎉', 'Account ban gaya! Humne verification email bhi bhej diya hai — inbox check kar lena.', 'success'); }).catch(e => { Swal.fire('Error', e.message, 'error'); if(btn){btn.innerHTML='📝 Register Karein';btn.disabled=false;} }); playSound('click'); }
...
function loginWithGoogle() {
    ...
    auth.signInWithPopup(provider).catch(e => {
        // User just closing the popup isn't an error worth alarming them about.
        if (e.code === 'auth/popup-closed-by-user' || e.code === 'auth/cancelled-popup-request') return;
        if (e.code === 'auth/popup-blocked') {
            Swal.fire('Popup Blocked', 'Browser ne popup block kar diya. Popup allow karke dobara try karein.', 'warning');
        } else if (e.code === 'auth/account-exists-with-different-credential') {
            Swal.fire('Account Already Exists', 'Ye email pehle se email/password se registered hai. Usi se login karein.', 'info');
        } else {
...
function forgotPassword() {
    ...
    auth.sendPasswordResetEmail(email)
        .then(() => Swal.fire('Bheja Gaya! 📧', 'Password reset link aapke email par bheja gaya!', 'success'))
        .catch(e => {
            let msg = e.message;
            if (e.code === 'auth/user-not-found') msg = 'Is email se koi account nahi mila.';
            else if (e.code === 'auth/invalid-email') msg = 'Email format sahi nahi hai.';
            else if (e.code === 'auth/too-many-requests') msg = 'Bahut zyada try ho gaya. Thodi der baad try karein.';
...
// Phase 3.1: tracks whether a saveToCloud() write is currently in flight.
// Real risk this closes: same account open on two devices/tabs — if the
// real-time listener below applies an incoming snapshot WHILE a local
// save is still being written, it can overwrite the in-memory arrays
// with older server data, silently discarding whatever the local save
// was about to persist. Full protection needs the sub-collection
// migration (roadmap Phase 3.2); this is the safe, minimal mitigation
// that fits today's single-document structure.
let _isSaving = false;
...
function loadCloudData(uid) {
    ...
    try {
        db.collection('familyData').doc(uid).onSnapshot((doc) => {
            if (_isSaving) return; // see _isSaving comment above
            if (doc.exists) {
                const data = doc.data(); 
...
async function saveToCloud() { 
    ...
    // Phase 6.2 fix: previously this returned early when offline, never
    // even calling Firestore. Now that db.enablePersistence() is on, the
    // write below is safe to attempt while offline — Firestore queues it
    // in IndexedDB and syncs automatically once the connection returns.
    ...
    try {
        ...
        if (s) {
            if (isOffline) {
                // Write succeeded into the local offline queue — it's safe,
                // just not on the server yet. Say so honestly instead of
                // claiming "Synced" for something still pending.
                s.innerText = "📥 Saved Offline"; s.style.color = "#f59e0b";
            } else {
                s.innerText = "☁️ Synced"; s.style.color = "#10b981"; setTimeout(() => s.style.color = "#94a3b8", 2000);
            }
        }
    } catch(e){
        // Phase 3.5 fix: this used to be a silent catch(e){} — a failed
        // save meant the user's latest changes stayed local with zero
        // indication anything went wrong. Now it's visible and honest.
        // With offline persistence on, genuine offline writes no longer
        // land here — only real failures (permissions, quota, etc.) do.
        console.error('saveToCloud failed:', e);
        const s = document.getElementById('sync-status');
        if (s) { s.innerText = "⚠️ Sync Failed"; s.style.color = "#ef4444"; }
        Swal.fire({ toast: true, position: 'top', icon: 'warning', title: 'Cloud sync fail hui — abhi data sirf isi device pe hai.', showConfirmButton: false, timer: 3500 });
    } finally {
        _isSaving = false;
    }
}
...
// Phase 7.2 fix: Tesseract.js is loaded on-demand now (it's a heavy
// ~2-3MB library including WASM) instead of on every page visit.
// This loads it once, the first time scanReceipt() is actually used,
// and reuses the same in-flight promise if called again quickly.
...
function _ensureTesseractLoaded() {
    ...
    _tesseractLoadPromise = new Promise((resolve, reject) => {
        ...
        script.onerror = () => { _tesseractLoadPromise = null; reject(new Error('Tesseract failed to load')); };
...
async function scanReceipt() {
    if(!currentReceiptUrl) return Swal.fire('Wait', 'Pehle receipt select karein.', 'info');
    const btn = document.getElementById('scan-btn');
    const originalLabel = btn.innerText;
    btn.innerText = "⏳ OCR Load Ho Rahi Hai...";
    try {
        await _ensureTesseractLoaded();
    } catch (loadErr) {
        btn.innerText = originalLabel;
        return Swal.fire('Error', 'OCR library load nahi ho payi — internet check karke dobara try karein.', 'error');
    }
    btn.innerText = "⏳ AI Scanning..."; Tesseract.recognize(currentReceiptUrl, 'eng').then(({ data }) => { const text = data.text; const confidence = data.confidence; /* Phase 4.2 fix: previously only matched ₹XX.XX (decimal required), so a bill printed as "Total: ₹500" (no paise) never matched. Now also catches whole-number amounts, still preferring 2-decimal matches since those are almost always real bill totals rather than dates/phone numbers. */ let amounts = text.match(/[\d,]+\.\d{2}/g); if(!amounts || !amounts.length) { amounts = text.match(/(?:₹|Rs\.?|INR)\s?[\d,]{3,}/gi); if(amounts) amounts = amounts.map(a => a.replace(/[₹RsINR.\s]/gi, '')); } if(amounts && amounts.length) { let maxAmt = Math.max(...amounts.map(a => parseFloat(a.replace(/,/g, '')))); document.getElementById('amount').value = maxAmt; /* Phase 4.1 fix: Tesseract's own confidence score is now checked — low-confidence reads (blurry/angled photos) still fill the amount in for convenience, but warn the user to double check it instead of presenting it as certain. */ if (confidence < 60) { Swal.fire('Scan Ho Gaya, Par Check Kar Lo 🔍', `Bill thoda unclear tha (confidence ${Math.round(confidence)}%). Amount ₹${maxAmt} nikala hai — sahi hai ya nahi verify kar lo.`, 'warning'); } else { Swal.fire('Scan Success! 📸', `Bill me se Amount ₹${maxAmt} nikal liya gaya!`, 'success'); } playSound('success'); } else { Swal.fire('Oops', 'Bill mein exact amount nahi mila. Khud daal lijiye.', 'info'); } btn.innerText = "📸 AI Scan"; }).catch(err => { btn.innerText = "📸 AI Scan"; Swal.fire('Error', 'Scanning fail ho gayi.', 'error'); }); playSound('click');
}
...
function importBankCSV() {
    const fileInput = document.getElementById('bank-csv-file');
    if(!fileInput.files.length) return Swal.fire('File Missing', 'Pehle ek CSV file upload karein!', 'warning');
    const file = fileInput.files[0];
    // Phase 5.3 fix: cap file size so a huge/wrong file can't freeze the tab
    if (file.size > 2 * 1024 * 1024) {
        return Swal.fire('File Bahut Badi', 'CSV file 2MB se chhoti honi chahiye.', 'warning');
    }
    const reader = new FileReader();
    reader.onload = function(e) {
        // Phase 5.3 fix: handle both \n and \r\n line endings, and skip blank lines
        const lines = e.target.result.split(/\r\n|\n/).filter(l => l.trim().length > 0);
        const MAX_ROWS = 2000; // safety cap so a huge file can't freeze the UI
        const rowsToProcess = Math.min(lines.length, MAX_ROWS + 1); // +1 to skip header row
        let added = 0, skipped = 0;
        for (let i = 1; i < rowsToProcess; i++) {
            // Phase 5.3 fix: quoted-field-aware split — previously a plain
            // .split(',') broke on descriptions containing commas
            // (e.g. "Groceries, Big Bazaar",450.00)
            const cols = lines[i].match(/(".*?"|[^,]+)(?=,|$)/g);
            if (!cols || cols.length < 3) { skipped++; continue; }
            const clean = cols.map(c => c.trim().replace(/^"|"$/g, ''));
            const dateVal = clean[0];
            const descVal = clean[1];
            const amtVal = parseFloat((clean[2] || clean[3] || '0').replace(/[^\d.-]/g, ''));
            if (isNaN(amtVal) || amtVal <= 0 || !descVal || descVal.length < 2) { skipped++; continue; }
            let fDate = todayDateString;
            let d = new Date(dateVal);
            if (!isNaN(d)) fDate = d.toISOString().split('T')[0];
            familyExpenses.push({ member: "Me", category: "Other", description: `🏦 Bank: ${descVal.substring(0,15)}`, amount: amtVal, date: fDate, receipt: "", gps: null });
            added++;
        }
        if (added > 0) {
            saveToCloud(); updateHisabUI(); playSound('success');
            let msg = `${added} naye kharche bank statement se jod diye gaye!`;
            if (skipped > 0) msg += ` (${skipped} rows skip hui — format sahi nahi tha)`;
            if (lines.length - 1 > MAX_ROWS) msg += ` Sirf pehli ${MAX_ROWS} rows process hui.`;
            Swal.fire('Imported! 🏦', msg, 'success');
        } else {
            Swal.fire('Error', `File format sahi nahi hai — ${skipped} rows try ki, koi valid expense nahi mila. Format: Date,Description,Amount`, 'error');
        }
        fileInput.value = "";
    };
...
function updateHisabUI() {
    ...
    // Phase 8.3 fix: familyExpenses.indexOf(item) used to run inside the
    // render loop below — a full linear scan of the whole array for
    // EVERY row rendered. With 500+ expenses that's up to ~250,000
    // comparisons on every single render (every filter/search/sort
    // change). Building this lookup once here makes each row's index
    // lookup O(1) instead.
    const expenseIndexMap = new Map();
    familyExpenses.forEach((exp, idx) => expenseIndexMap.set(exp, idx));
    ...
    if(filteredExpenses.length === 0) {
        ...
        filteredExpenses.forEach((item) => {
            ...
            const originalIndex = expenseIndexMap.get(item); const li = document.createElement('li');
...
// Phase 7.4 fix: the ration search box used to call updateRationUI()
// (a full list.innerHTML rebuild) on every single keystroke. This
// debounces it the same way smartSearch already does elsewhere —
// waits 200ms after the user stops typing before re-rendering.
let _rationSearchTimeout;
function debouncedRationSearch() {
    clearTimeout(_rationSearchTimeout);
    _rationSearchTimeout = setTimeout(updateRationUI, 200);
}
...
// ==========================================
// 🔒 REAL BACKUP ENCRYPTION (Phase 5 fix)
// Previously "backupData" only did btoa() — base64 encoding, not
// encryption. Anyone could decode it with one line of JS. This now
// uses actual AES-GCM with a password-derived key (PBKDF2, 150k
// iterations), so a backup file is genuinely unreadable without the
// password the user set when creating it.
// ==========================================
async function _gmDeriveKey(passphrase, saltBytes) {
...
function _gmBufToB64(buf) { return btoa(String.fromCharCode(...new Uint8Array(buf))); }
function _gmB64ToBuf(b64) { return Uint8Array.from(atob(b64), c => c.charCodeAt(0)); }

async function backupData() {
    const dataToBackup = { expenses: familyExpenses, dudh: dudhRecords, ration: rationItems, investments: investments, loans: activeLoans, subscriptions: activeSubs, recharges: rechargeRecords, budget: budgetLimit, income: monthlyIncome, xp: userXP, dailyStreak: dailyStreak, todoItems: todoItems, dreamGoal: dreamGoal };

    const { value: passphrase } = await Swal.fire({
        title: '🔐 Backup Password Banao',
        html: '<p style="font-size:13px;color:#64748b;">Ye password backup file ko lock karega. Yaad rakhna — restore karte waqt yehi password chahiye hoga, bhool gaye to backup wapas nahi khulega.</p>',
        input: 'password',
        inputPlaceholder: 'Kam se kam 4 characters',
        showCancelButton: true,
        confirmButtonText: 'Backup Banao 🔒',
        confirmButtonColor: '#7c3aed',
        background: 'var(--paper-bg)', color: 'var(--text-main)',
        inputValidator: (v) => (!v || v.length < 4) ? 'Kam se kam 4 characters ka password rakho' : undefined
    });
    if (!passphrase) return;

    const salt = crypto.getRandomValues(new Uint8Array(16));
    const iv = crypto.getRandomValues(new Uint8Array(12));
    const key = await _gmDeriveKey(passphrase, salt);
    const plainBytes = new TextEncoder().encode(JSON.stringify(dataToBackup));
    const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, plainBytes);

    const payload = { gharmanager_encrypted: true, v: 1, salt: _gmBufToB64(salt), iv: _gmBufToB64(iv), data: _gmBufToB64(cipherBuf) };
    const dataStr = "data:application/json;charset=utf-8," + encodeURIComponent(JSON.stringify(payload));
    const dlAnchorElem = document.createElement('a');
    dlAnchorElem.setAttribute("href", dataStr);
    dlAnchorElem.setAttribute("download", "GharManager_Encrypted_Backup.json");
    dlAnchorElem.click();
    playSound('success');
    Swal.fire({ toast: true, position: 'top', icon: 'success', title: 'Encrypted backup ban gaya! 🔒', timer: 2500, showConfirmButton: false });
}
...
function restoreData(event) {
    ...
    reader.onload = async function(e) {
        ...
        try {
            let data = null;
            let parsed = null;
            try { parsed = JSON.parse(raw); } catch (err) { /* not JSON — might be a legacy base64 backup, handled below */ }

            if (parsed && parsed.gharmanager_encrypted) {
                // New real-encrypted format — ask for the password
                const { value: passphrase } = await Swal.fire({
                    title: '🔐 Backup Password Daalo',
                    input: 'password',
                    inputPlaceholder: 'Backup banate waqt jo password diya tha',
                    showCancelButton: true,
                ...
                try {
                    const salt = _gmB64ToBuf(parsed.salt);
                    const iv = _gmB64ToBuf(parsed.iv);
                    const key = await _gmDeriveKey(passphrase, salt);
                    const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, key, _gmB64ToBuf(parsed.data));
                    data = JSON.parse(new TextDecoder().decode(plainBuf));
                } catch (decErr) {
                    Swal.fire('Galat Password', 'Password match nahi hua — dobara try karo.', 'error');
                    event.target.value = ""; return;
                }
            } else {
                // Legacy backups from before Phase 5 (plain base64, no real
                // encryption) — still restorable so old backup files don't break.
                let decodedStr = raw;
                try { decodedStr = decodeURIComponent(escape(atob(raw))); } catch (err) {}
...
const _OR_CONFIG = {
    // apiKey/endpoint/model REMOVED from here on purpose (Phase 1 security fix).
    // The OpenRouter key now lives only in the Cloud Function's environment
    // config — it never reaches the browser. See functions/index.js.
    maxRetries: 3,
    retryDelayMs: 1500,
};
...
async function _orCallAPI(messages, attempt) {
    ...
    // Phase 1 security fix: the browser no longer talks to OpenRouter
    // directly (that required shipping a secret key to every visitor).
    // Instead it calls our own authenticated Cloud Function, which holds
    // the real key server-side and applies a daily per-user rate limit.
    let result;
    try {
        const callAskAI = firebase.functions().httpsCallable('askAI');
        result = await callAskAI({ messages: messages });
    } catch (err) {
        const code = err && err.code; // e.g. 'functions/unauthenticated'

        if (code === 'functions/unauthenticated') {
            throw new Error('HTTP_401:Login required');
        }
        if (code === 'functions/resource-exhausted') {
            throw new Error('HTTP_429:' + (err.message || 'Rate limited'));
        }
        if (code === 'functions/unavailable' || code === 'functions/internal' || code === 'functions/deadline-exceeded') {
            // Transient server-side failure — retry like the old 5xx path did
            ...
            throw new Error('HTTP_500:' + (err.message || 'Server error'));
        }

        // Anything else (offline, DNS, function not reachable) — treat as
        // a pure network failure, same retry behaviour as before.
        if (attempt < _OR_CONFIG.maxRetries) {
            await new Promise(r => setTimeout(r, _OR_CONFIG.retryDelayMs * attempt));
            return _orCallAPI(messages, attempt + 1);
        }
    ...
    const text = result && result.data && result.data.reply;
...
function startVoice() { 
    ...
    try {
        ...
        // Phase 4.3 fix: previously hardcoded to 'hi-IN' regardless of the
        // user's app language preference. Now it follows the same
        // appLang toggle already used everywhere else in the app —
        // 'English' -> en-IN, 'Hinglish' (default) -> hi-IN.
        recognition.lang = (appLang === 'English') ? 'en-IN' : 'hi-IN';
...
function removeMember(index) {
    ...
    Swal.fire({ title: `Remove ${name}?`, text: 'Ye member family se remove ho jayega.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'Remove' }).then(res => {
        if (res.isConfirmed) {
            ...
            saveFamilyMembers();
            openMembersPanel();
            updateMemberDropdowns(); // Phase 3.4 fix: previously the removed member stayed selectable in the Add Expense dropdown until a page reload
        }
    });
}
