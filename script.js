...
let cloudDataLoaded = false; // pehli baar Firestore se response (data ya "no doc") aane ke baad true
let cloudDataReadyPromiseResolve = null;
let cloudDataReadyPromise = new Promise(res => { cloudDataReadyPromiseResolve = res; });

// ==========================================
// 🐞 ON-SCREEN CLOUD DEBUG PANEL (no computer/console needed)
// App ke corner me ek chhota box dikhata hai jisme UID, cloud status,
// aur error clearly likha hota hai. Hataane ke liye is poore block ko
// aur jahan-jahan cloudDebug(...) call ho raha hai use delete kar dein.
// ==========================================
function ensureCloudDebugBox() {
    let box = document.getElementById('cloud-debug-box');
    if (box) return box;
    box = document.createElement('div');
    box.id = 'cloud-debug-box';
    box.style.cssText = 'position:fixed; bottom:8px; left:8px; right:8px; z-index:999999; background:rgba(15,23,42,0.95); color:#e2e8f0; font:11px/1.5 monospace; padding:10px 12px; border-radius:10px; max-height:38vh; overflow:auto; border:1px solid #334155; box-shadow:0 4px 20px rgba(0,0,0,0.4);';
    const closeBtn = document.createElement('div');
    closeBtn.innerText = '✖ Debug band karein';
    closeBtn.style.cssText = 'position:sticky; top:0; text-align:right; color:#f87171; cursor:pointer; font-weight:bold; margin-bottom:6px;';
    closeBtn.onclick = () => box.remove();
    box.appendChild(closeBtn);
    const content = document.createElement('div');
    content.id = 'cloud-debug-content';
    box.appendChild(content);
    document.body.appendChild(box);
    return box;
}

function cloudDebug(line, color) {
    ensureCloudDebugBox();
    const content = document.getElementById('cloud-debug-content');
    const row = document.createElement('div');
    row.style.color = color || '#94a3b8';
    row.innerText = new Date().toLocaleTimeString() + ' — ' + line;
...
function loadCloudData(uid) {
    if(isOffline) return;
    if (!uid) { console.error("[Cloud] loadCloudData bina uid ke call hua — skip"); return; }
    cloudDebug('Login UID: ' + uid, '#38bdf8');
    cloudDebug('Email: ' + (currentUser ? currentUser.email : 'unknown'), '#38bdf8');
    try {
        db.collection('familyData').doc(uid).onSnapshot((doc) => {
            if (doc.exists) {
                ...
                console.log("[Cloud] Data mil gaya — UID:", uid, "| Expenses:", familyExpenses.length);
                cloudDebug('Doc Exists: ✅ Haan', '#34d399');
                cloudDebug('Expenses mile: ' + familyExpenses.length, '#34d399');
            } else {
                console.warn("[Cloud] familyData/" + uid + " document maujood NAHI hai. Agar cloud pe data hone ka pura yakeen hai, to shayad app me is waqt kisi DOOSRE account se login hai (UID mismatch).");
                cloudDebug('Doc Exists: ❌ NAHI — ye UID Firebase Console ke document ID se match nahi karta, ya naya account hai.', '#fbbf24');
            }
            if (!cloudDataLoaded) { cloudDataLoaded = true; cloudDataReadyPromiseResolve(); }
        }, (error) => {
            // 🚨 Pehle yahan koi error-handler hi nahi tha — Firestore permission/read errors CHUPCHAP fail ho jaate the, isliye app khaali dikhta tha lekin console me bhi kuch nahi aata tha.
            console.error("[Cloud] Firestore read FAILED:", error.code, error.message);
            cloudDebug('🚨 ERROR: ' + error.code + ' — ' + error.message, '#f87171');
            if (!cloudDataLoaded) { cloudDataLoaded = true; cloudDataReadyPromiseResolve(); }
            if (error.code === 'permission-denied') {
                Swal.fire({
                    icon: 'error',
                    title: 'Cloud se data load nahi ho paaya',
                    html: 'Firestore Security Rules ki wajah se data read block ho raha hai (permission-denied).<br><br>Firebase Console → Firestore Database → Rules me check karein ki logged-in user apna khud ka document (<code>familyData/{uid}</code>) padh sakta ho.'
                });
            } else {
                Swal.fire({ icon: 'error', title: 'Cloud Sync Error', text: error.message });
            }
        });
...
async function syncOldLocalData() { 
    if(isOffline) return;
    // 🔒 Zaroori: pehle cloud snapshot ka wait karo, warna purana/khaali localStorage cache
    // real cloud data ke upar overwrite ho sakta hai (race condition).
    await cloudDataReadyPromise;
