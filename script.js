    ...
    if(filteredExpenses.length === 0) {
        ...
        filteredExpenses.forEach((item) => {
            if(isDateSorted && item.date !== currentDateHeader) {
                ...
                const dateHeader = document.createElement('div'); dateHeader.className = 'hx-date-hdr'; dateHeader.innerHTML = `<span class="hx-date-hdr-dot"></span>${showDate}`; fragment.appendChild(dateHeader);
            }
            const originalIndex = familyExpenses.indexOf(item); const li = document.createElement('li');
            li.className = 'hx-li-item';
            const catMeta = getCategoryMeta(item.category);
            const monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
            const dateLabel = item.date ? `${parseInt(item.date.slice(8))} ${monthNames[parseInt(item.date.slice(5,7))-1]}` : '';
            const thumbHTML = item.receipt ? `<img src="${item.receipt}" class="hx-li-thumb" onclick="event.stopPropagation();Swal.fire({imageUrl:'${item.receipt}',imageWidth:'100%'})">` : '';
            li.setAttribute('ondblclick', `editExpense(${originalIndex})`);
            li.innerHTML = `
              <div class="hx-li-icon" style="background:${catMeta.bg};color:${catMeta.color};border:1.5px solid ${catMeta.border||catMeta.bg};">${catMeta.icon}</div>
              <div class="hx-li-body">
                <p class="hx-li-title">${item.description}</p>
                <div class="hx-li-meta">
                  <span class="hx-li-cat-pill" style="background:${catMeta.bg};color:${catMeta.color};">${item.category||'Other'}</span>
                  <span class="hx-li-dot">·</span>
                  <span class="hx-li-mbr">👤 ${item.member}</span>
                  ${dateLabel ? `<span class="hx-li-dot">·</span><span class="hx-li-date">${dateLabel}</span>` : ''}
                </div>
              </div>
              <div class="hx-li-right">
                ${thumbHTML}
                <span class="hx-li-amt" style="color:${catMeta.color};">₹${item.amount}</span>
                <button class="hx-li-del" onclick="event.stopPropagation();deleteExpense(${originalIndex})" title="Delete"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2.5" width="14" height="14"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/></svg></button>
              </div>`;
...
function renderRationItem(list, item, index) {
    ...
    const boughtClass = item.bought ? ' rt-bought' : '';
    const lowClass    = item.lowStock ? ' rt-low' : '';
    li.className = `rt-item${boughtClass}${lowClass}`;
    ...
    const trendHTML = priceTrend
        ? `<span class="${priceTrend.includes('🔴') ? 'rt-trend-up' : 'rt-trend-down'}">${priceTrend}</span>`
        : '';
    li.innerHTML = `
      <div class="rt-chk ${item.bought ? 'rt-chk-done' : ''}" onclick="toggleRation(${index})">${item.bought ? checkSVG : ''}</div>
      <div class="rt-body" onclick="toggleRation(${index})">
        <p class="rt-name">${item.name}</p>
        <div class="rt-tags">
          ${qty ? `<span class="rt-qty" style="background:${cc.bg};color:${cc.text};">${qty}</span>` : ''}
          <span class="rt-cat" style="background:${cc.bg};color:${cc.text};">${cc.icon} ${cat}</span>
          ${item.lowStock ? '<span class="rt-low-tag">⚠️ Low</span>' : ''}
          ${item.note ? `<span class="rt-note">· ${item.note}</span>` : ''}
        </div>
      </div>
      <div class="rt-right">
        ${item.amount > 0 ? `<span class="rt-price">₹${item.amount}</span>` : ''}
        ${trendHTML}
        <div class="rt-actions">
          <button type="button" class="rt-btn rt-btn-edit" onclick="editRationPrice(${index})" title="Edit Price">₹</button>
          <button type="button" class="rt-btn rt-btn-low"  onclick="toggleLowStock(${index})" title="Low Stock">⚠️</button>
          <button type="button" class="rt-btn rt-btn-del"  onclick="deleteRation(${index})"   title="Delete">🗑️</button>
        </div>
      </div>`;
    list.appendChild(li);
}
