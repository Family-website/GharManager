...
function addExpense() {
    try {
        ...
        if (typeof currentReceiptUrl !== 'undefined') currentReceiptUrl = "";
...
// 🛡️ 10. REAL GROQ (LLAMA 3) AI CHAT & VOICE
function startVoice() { 
...
function openAIBottomSheet() {
    Swal.fire({
        ...
        html: `<div style="display:flex; flex-direction:column; gap:10px;">
                ...
                <p style="font-size:12px; font-weight:bold; color:var(--ink-blue); text-align:left; margin:0;">💬 Chat with AI (Groq Fast)</p>
                <div id="ai-chat-box" style="background:var(--line-color); height:150px; border-radius:12px; padding:10px; font-size:12px; overflow-y:auto; text-align:left; margin-bottom:5px;">
                    Hello! Mujhse apne kharcho ke baare me poocho...
                </div>
                <div style="display:flex; gap:8px; align-items:center;">
                    <input type="text" id="ai-user-input" placeholder="Type here..." style="flex:1; width:100%; min-height:40px; padding:8px 12px; border-radius:8px; border:1px solid #cbd5e1; font-size:14px; background:var(--paper-bg); color:var(--text-main); box-sizing: border-box;">
                    <button onclick="sendToRealAI()" style="background:#10b981; color:white; border:none; padding:10px 15px; border-radius:8px; font-weight:bold; cursor:pointer;">Send</button>
                </div>
               </div>`,
...
async function sendToRealAI() {
    // 🔥 YAHAN TUMHARI GROQ API KEY HARDCODE KAR DI GAYI HAI 🔥
    const GROQ_API_KEY = "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"; 
    ...
    chatBox.innerHTML += `<div style="margin:5px 0; text-align:right;"><span style="background:#3b82f6; color:white; padding:6px 10px; border-radius:12px; display:inline-block; max-width:80%; word-wrap:break-word;">${userText}</span></div>`;
    ...
    let systemMsg = `Tum GharManager ke ek smart AI finance assistant ho. User ka is mahine ka total kharcha ₹${totalExp} hai aur kamai ₹${monthlyIncome} hai. Kharcho ki list: ${expensesList}. User ke sawal ka chhota aur point-to-point jawab Roman Hindi (Hinglish) me do. Markdown formatting jaise asterisks (**) ka use mat karna.`;

    let loadingId = 'load-' + Date.now();
    chatBox.innerHTML += `<div id="${loadingId}" style="margin:5px 0; text-align:left;"><span style="background:var(--paper-bg); color:var(--text-main); border:1px solid #cbd5e1; padding:6px 10px; border-radius:12px; display:inline-block;">Typing (Groq Fast AI)... ⚡</span></div>`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:6px 10px; border-radius:12px; display:inline-block; font-size:11px;"><b>API Error:</b> ${errMsg}</span></div>`;
            chatBox.scrollTop = chatBox.scrollHeight;
            return;
        }
        ...
        if (data.choices && data.choices[0].message) {
            ...
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:var(--paper-bg); color:var(--text-main); border:1px solid #cbd5e1; padding:6px 10px; border-radius:12px; display:inline-block; max-width:90%; word-wrap:break-word;">${aiText}</span></div>`;
        } else {
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:6px 10px; border-radius:12px; display:inline-block;">AI ne koi jawab nahi diya.</span></div>`;
        }
        
        chatBox.scrollTop = chatBox.scrollHeight; playSound('success');
    } catch(e) {
        if(document.getElementById(loadingId)) document.getElementById(loadingId).remove();
        chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:6px 10px; border-radius:12px; display:inline-block; font-size:11px;">Internet ya Network Error!</span></div>`;
