...
// Helper: Force FontAwesome Render for Mobile Editors
function renderIcons() { if(window.FontAwesome) { FontAwesome.dom.i2svg(); } }
...
function updateSoundUI() { const btn = document.getElementById('sound-toggle-btn'); if(btn) { btn.innerHTML = isSoundEnabled ? '<i class="fa-solid fa-volume-high"></i> Sound: ON' : '<i class="fa-solid fa-volume-xmark"></i> Sound: OFF'; btn.style.color = isSoundEnabled ? '#10b981' : '#64748b'; btn.style.borderColor = isSoundEnabled ? '#10b981' : '#64748b'; renderIcons(); } }
function playSound(type) { if(navigator.vibrate) try { navigator.vibrate(type === 'success' ? [30, 50, 30] : 30); } catch(e){} if(!isSoundEnabled) return; try { if(type === 'click') document.getElementById('sound-click').play(); if(type === 'success') document.getElementById('sound-success').play(); } catch(e) {} }
...
function checkRole(name) { 
    ...
    if(n.includes('aditya') || n.includes('papa')) { 
        userRole = 'Admin'; 
        let b = document.getElementById('role-badge'); if(b){ b.innerHTML = '<i class="fa-solid fa-crown"></i> Admin'; b.style.background = '#fef08a'; b.style.color = '#b45309'; } 
        // Show Admin Only Buttons
        if(document.getElementById('btn-edit-budget')) document.getElementById('btn-edit-budget').style.display = 'block';
        if(document.getElementById('btn-edit-income')) document.getElementById('btn-edit-income').style.display = 'block';
    } else { 
        userRole = 'Member'; 
        let b = document.getElementById('role-badge'); if(b){ b.innerHTML = '<i class="fa-solid fa-user"></i> Member'; b.style.background = '#e2e8f0'; b.style.color = '#475569'; } 
        // Hide Admin Only Buttons
        if(document.getElementById('btn-edit-budget')) document.getElementById('btn-edit-budget').style.display = 'none';
        if(document.getElementById('btn-edit-income')) document.getElementById('btn-edit-income').style.display = 'none';
    }
    renderIcons();
}
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        currentUser = user; 
        let hour = new Date().getHours(); let wish = hour < 12 ? 'Good Morning' : hour < 18 ? 'Good Afternoon' : 'Good Evening'; document.getElementById('smart-greeting').innerText = `${wish}`;
        let userName = user.email.split("@")[0]; document.getElementById('user-avatar').innerText = userName.charAt(0).toUpperCase(); checkRole(customDisplayName || userName);
...
function biometricUnlock() { let savedPin = localStorage.getItem('app_pin'); if(!savedPin) return Swal.fire('Wait', 'Pehle PIN setup karein!', 'info'); Swal.fire({ title: 'Scanning...', iconHtml: '<i class="fa-solid fa-fingerprint"></i>', timer: 1500, timerProgressBar: true, showConfirmButton: false }).then(() => { document.getElementById('pin-screen').style.display = 'none'; document.getElementById('main-app').style.display = 'block'; playSound('success'); }); }

// ==========================================
// CLOUD SYNC & LOCAL CACHE 
// ==========================================
function showSyncSuccess() { const syncEl = document.getElementById('sync-status'); if(syncEl) { syncEl.innerHTML = '<i class="fa-solid fa-cloud-check"></i> Synced Just Now'; syncEl.style.color = "#10b981"; setTimeout(() => { syncEl.style.color = "#94a3b8"; syncEl.innerHTML = '<i class="fa-solid fa-cloud"></i> Cloud Active'; renderIcons(); }, 3000); renderIcons(); } }
...
async function saveToCloud() { 
    localStorage.setItem('familyExpenses', JSON.stringify(familyExpenses)); localStorage.setItem('dudhRecords', JSON.stringify(dudhRecords)); localStorage.setItem('rationItems', JSON.stringify(rationItems)); localStorage.setItem('investments', JSON.stringify(investments)); localStorage.setItem('loans', JSON.stringify(activeLoans)); localStorage.setItem('subscriptions', JSON.stringify(activeSubs)); localStorage.setItem('rechargeRecords', JSON.stringify(rechargeRecords));
    localStorage.setItem('appDataCache', JSON.stringify({ budget: budgetLimit, income: monthlyIncome, xp: userXP, challengeDays: challengeDays, dailyStreak: dailyStreak, lastLoginDate: lastLoginDate, displayName: customDisplayName, dreamGoal: dreamGoal }));
    
    if(!currentUser) return; if(isOffline) return; 
    try {
        ...
        const s = document.getElementById('sync-status'); if(s) { s.innerHTML = '<i class="fa-solid fa-cloud-check"></i> Synced'; s.style.color = "#10b981"; setTimeout(() => { s.style.color = "#94a3b8"; s.innerHTML = '<i class="fa-solid fa-cloud"></i> Synced'; renderIcons(); }, 2000); renderIcons(); }
    } catch(e){}
}

async function syncOldLocalData() { if(isOffline) return; let dataChanged = false; if (familyExpenses.length > 0) { dataChanged = true; } if (dataChanged) { await saveToCloud(); } }
...
document.addEventListener('touchend', e => {
    ...
    if(pEndY > pStartY + 150 && window.scrollY === 0 && currentUser) {
        playSound('click'); pRef.style.display = 'block'; pRef.innerHTML = '<i class="fa-solid fa-rotate fa-spin"></i> Syncing...'; renderIcons();
        setTimeout(() => { if(!isOffline) loadCloudData(currentUser.uid); pRef.innerHTML = '<i class="fa-solid fa-check"></i> Synced!'; renderIcons(); playSound('success'); setTimeout(() => pRef.style.display = 'none', 1000); }, 1000);
    }
});
...
function updateRechargeUI() {
    ...
    rechargeRecords.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach((rech, index) => {
        ...
        else if(diffDays <= 3) { statusHtml = `<span style="color:#f59e0b; font-weight:bold; font-size:11px;">Expiring in ${diffDays} days</span>`; borderCol = "#f59e0b"; }
...
// ==========================================
// 💎 PREMIUM USER PROFILE & NEW INVITE
// ==========================================
...
async function convertToUSD() {
    ...
    try {
        let res = await fetch('https://open.er-api.com/v6/latest/INR');
        let data = await res.json();
        let rate = data.rates.USD;
        let usdValue = (netWorth * rate).toFixed(2);
        Swal.fire('Net Worth', `Live Exchange Rate: 1 INR = $${rate}<br><br><b>$${usdValue} USD</b>`, 'success');
    } catch(e) {
        let fallbackUsd = (netWorth / 83.5).toFixed(2);
        Swal.fire('Net Worth (Offline)', `Approximate Value:<br><b>$${fallbackUsd} USD</b>`, 'info');
    }
    btn.innerHTML = originalText; renderIcons();
}

// 🔥 NEW: Invite System
function inviteFamily() {
    playSound('click');
    let msg = `GharManager Pro me judo! 🚀\nApna kharcha, budget aur bills family ke sath milkar smart tarike se manage karo.\n\n👇 Click to Join:\nhttps://family-website.github.io/GharManager/`;
    if(navigator.share) {
        navigator.share({ title: 'Join GharManager Pro', text: msg }).catch(console.error);
    } else {
        navigator.clipboard.writeText(msg);
        Swal.fire('Link Copied!', 'App ka link copy ho gaya hai. Ab ise WhatsApp par apni family ko bhej do.', 'success');
    }
}
...
function openProfile() {
    try {
        ...
        if(scoreBar && scoreText && scoreMsg) { 
            ...
            else { scoreBar.style.background = '#ef4444'; scoreText.style.color = '#ef4444'; scoreMsg.innerText = "Warning! Kharcha control se bahar hai."; } 
        }
...
function applyLanguageUI() { document.querySelectorAll('.translatable').forEach(el => { let key = el.getAttribute('data-key') || el.innerText.trim(); if(!el.getAttribute('data-key')) el.setAttribute('data-key', key); if(dict[appLang] && dict[appLang][key]) { if(el.children.length>0 && el.innerHTML.includes('<span')) { el.innerHTML = el.innerHTML.replace(key, dict[appLang][key]); } else { el.innerText = dict[appLang][key]; } } }); renderIcons(); }
...
// 🔥 FIXED: Heading Title Logic
function openSection(sName, title) { 
    document.querySelectorAll('.app-section').forEach(sec => sec.classList.remove('active-section')); 
    document.getElementById('section-'+sName).classList.add('active-section'); 
    document.getElementById('app-title').innerText = title; 
    document.querySelectorAll('.nav-btn').forEach(btn => { btn.classList.remove('active-nav'); if(btn.getAttribute('onclick').includes(`'${sName}'`)) btn.classList.add('active-nav'); }); 
...
function updatePrediction(tot) { 
    ...
    if (fm !== currM) { el.innerText = `Past Month`; el.style.color = '#64748b'; return; }
    let d = new Date().getDate(); if (d === 0) d = 1; let dim = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate(); 
    let p = (tot / d) * dim; if(isNaN(p) || !isFinite(p)) p = 0; 
    el.innerText = `₹${Math.round(p)} ${appLang==='English'?'Expected':'Umeed'}`; el.style.color = p > budgetLimit ? '#ef4444' : '#10b981'; 
}
...
function renderCalendar(exps, fm) {
    ...
    for(let i=1; i<=dim; i++) {
        ...
        if(dt[i]) { el.style.color = 'white'; if(dt[i] >= 1000) el.style.background = '#ef4444'; else if(dt[i] >= 300) el.style.background = '#f59e0b'; else el.style.background = '#10b981'; } else { el.style.background = 'var(--line-color)'; el.style.color = 'var(--text-main)'; }
        frag.appendChild(el);
    }
    cel.appendChild(frag);
}

function captureLocation() { let statusEl = document.getElementById('loc-status'); statusEl.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Fetching...'; renderIcons(); if (navigator.geolocation) { navigator.geolocation.getCurrentPosition((pos) => { currentGPSLocation = `https://www.google.com/maps/search/?api=1&query=$${pos.coords.latitude},${pos.coords.longitude}`; statusEl.innerHTML = '<i class="fa-solid fa-check"></i> Captured!'; statusEl.style.color = "#10b981"; playSound('click'); renderIcons(); }, () => { statusEl.innerHTML = '<i class="fa-solid fa-xmark"></i> Failed'; statusEl.style.color = "#ef4444"; Swal.fire('Error', 'Location on karein!', 'error'); renderIcons(); }); } else { statusEl.innerHTML = '<i class="fa-solid fa-xmark"></i> Not Supported'; renderIcons(); } playSound('click'); }
...
// 💰 HISAAB SECTION
// ==========================================
let editExpenseIndex = -1; let currentReceiptUrl = ""; 
...
function scanReceipt() { if(!currentReceiptUrl || typeof Tesseract === 'undefined') return Swal.fire('Wait', 'Library load ho rahi hai...', 'info'); const btn = document.getElementById('scan-btn'); btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Scanning...'; renderIcons(); Tesseract.recognize(currentReceiptUrl, 'eng').then(({ data: { text } }) => { let amounts = text.match(/[\d,]+\.\d{2}/g); if(amounts) { let maxAmt = Math.max(...amounts.map(a => parseFloat(a.replace(/,/g, '')))); document.getElementById('amount').value = maxAmt; Swal.fire('Success!', `Bill me se Amount ₹${maxAmt} nikal liya gaya!`, 'success'); playSound('success'); } else { Swal.fire('Oops', 'Bill mein exact amount nahi mila.', 'info'); } btn.innerHTML = '<i class="fa-solid fa-expand"></i> Scan'; renderIcons(); }).catch(err => { btn.innerHTML = '<i class="fa-solid fa-expand"></i> Scan'; renderIcons(); Swal.fire('Error', 'Scanning fail ho gayi.', 'error'); }); playSound('click'); }
...
function renderHistoryWithSkeleton() { const list = document.getElementById('history-list'); if(!list) return; list.innerHTML = `<div class="skeleton-box" style="height:60px; margin-bottom:10px;"></div><div class="skeleton-box" style="height:60px; margin-bottom:10px;"></div>`; setTimeout(updateHisabUI, 300); }
...
function updateHisabUI() {
    ...
    if(filteredExpenses.length === 0) {
        list.innerHTML = `<div style="text-align:center; padding: 40px 10px; opacity:0.7; background:var(--line-color); border-radius:15px;"><div style="font-size:50px; margin-bottom:10px; color:var(--text-muted);"><i class="fa-solid fa-folder-open"></i></div><h3 style="color:var(--text-main); font-size:18px;">No Records Found</h3><p style="color:var(--text-muted); font-size:13px; font-weight:bold;">Is filter ya mahine mein koi kharcha nahi mila</p></div>`;
    } else {
        let currentDateHeader = "";
        filteredExpenses.forEach((item) => {
            if(isDateSorted && item.date !== currentDateHeader) { currentDateHeader = item.date; const parts = currentDateHeader.split('-'); const dateObj = new Date(parts[0], parts[1] - 1, parts[2]); const showDate = `${dateObj.getDate()} ${['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][dateObj.getMonth()]}`; const dateHeader = document.createElement('div'); dateHeader.className = 'date-header'; dateHeader.style.fontWeight = 'bold'; dateHeader.style.color = 'var(--ink-blue)'; dateHeader.style.margin = '10px 0 5px 0'; dateHeader.innerHTML = `<i class="fa-regular fa-calendar"></i> ${showDate}`; list.appendChild(dateHeader); }
            const originalIndex = familyExpenses.indexOf(item); const li = document.createElement('li');
            
            // 🔥 ADVANCED ADMIN ROLE DELETE BUTTON
            let deleteBtnHtml = "";
            let safeName = customDisplayName || (currentUser?currentUser.email.split("@")[0]:"");
            if (userRole === 'Admin' || item.member.toLowerCase() === safeName.toLowerCase()) {
                deleteBtnHtml = `<button class="action-btn delete" onclick="event.stopPropagation(); deleteExpense(${originalIndex})"><i class="fa-solid fa-trash"></i></button>`;
            } else {
                deleteBtnHtml = `<button class="action-btn" style="opacity:0.3; cursor:not-allowed;" onclick="event.stopPropagation(); Swal.fire('Locked', 'Only Admin or the owner can delete this.', 'error');"><i class="fa-solid fa-lock"></i></button>`;
            }

            li.setAttribute('ondblclick', `editExpense(${originalIndex})`); li.style.cursor = 'pointer'; 
            li.innerHTML = `<div class="list-left" style="pointer-events:none;"><strong style="font-size: 18px;">${item.description}</strong><div style="display: flex; align-items: center; margin-top: 5px; flex-wrap: wrap; gap: 5px;"><span class="member-badge"><i class="fa-solid fa-user"></i> ${item.member}</span> <span class="category-badge">${item.category||'Other'}</span></div></div><div class="list-right"><span style="font-weight: 800; color: #e74c3c; font-size: 20px; margin: 0 5px;">₹${item.amount}</span>${deleteBtnHtml}</div>`;
            list.appendChild(li);
        });
    }
    ...
    if(compEl) {
        ...
        if(lastMonthTotal > 0) {
            ...
            if(diff > 0) { compEl.innerHTML = `<i class="fa-solid fa-arrow-trend-up"></i> Pichle mahine se <b>${percentDiff}% zyada</b> kharcha hua.`; compEl.style.color = '#ef4444'; } else if(diff < 0) { compEl.innerHTML = `<i class="fa-solid fa-arrow-trend-down"></i> Pichle mahine se <b>${percentDiff}% kam</b> kharcha hua!`; compEl.style.color = '#10b981'; } else { compEl.innerHTML = `<i class="fa-solid fa-minus"></i> Pichle mahine jitna hi kharcha chal raha hai.`; compEl.style.color = '#64748b'; }
        } else { compEl.innerHTML = `<i class="fa-solid fa-chart-line"></i> Abhi tak ka kharcha: ₹${totalExpense}`; compEl.style.color = '#64748b'; }
    }
    ...
    let fm = document.getElementById('month-filter') ? document.getElementById('month-filter').value : todayDateString.slice(0, 7);
    let currM = todayDateString.slice(0, 7);
    const pEl = document.getElementById('predicted-expense');
    if (pEl) {
        if (fm !== currM) { pEl.innerText = `Past Month`; pEl.style.color = '#64748b'; }
        else {
            let d = new Date().getDate(); if (d === 0) d = 1; let dim = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate(); 
            let p = (totalExpense / d) * dim; if(isNaN(p) || !isFinite(p)) p = 0; 
            pEl.innerText = `₹${Math.round(p)} ${appLang==='English'?'Expected':'Umeed'}`; pEl.style.color = p > budgetLimit ? '#ef4444' : '#10b981'; 
        }
    }
    
    renderCalendar(filteredExpenses, filterMonth); applyLanguageUI(); renderIcons();
}
...
function addExpense() {
    ...
    else { familyExpenses[editExpenseIndex] = newRecord; editExpenseIndex = -1; document.getElementById('btn-add-expense').innerHTML = '<i class="fa-solid fa-plus"></i> Add Kharcha'; Swal.fire({toast:true, position:'top-end', icon:'success', title:'Updated!', showConfirmButton:false, timer:1500}); }
    currentGPSLocation = null; let statusEl = document.getElementById('loc-status'); if(statusEl) { statusEl.innerHTML = '<i class="fa-solid fa-location-dot"></i> Not Saved'; statusEl.style.color = "var(--text-muted)"; }
...
function updateDudhUI() { const list = document.getElementById('dudh-list'); if(!list) return; list.innerHTML = ''; let totalLiter = 0, totalBill = 0; if(dudhRecords.length === 0) list.innerHTML = `<div style="text-align:center; padding: 20px; opacity:0.6;"><h3 style="color:var(--text-main); font-size:14px;">No Milk Records</h3></div>`; dudhRecords.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach((record, index) => { const totalDayLiter = record.morning + record.evening; const dayCost = totalDayLiter * record.rate; totalLiter += totalDayLiter; totalBill += dayCost; const parts = record.date.split('-'); const dateObj = new Date(parts[0], parts[1] - 1, parts[2]); const showDate = `${dateObj.getDate()} ${['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][dateObj.getMonth()]}`; const li = document.createElement('li'); li.innerHTML = `<div class="list-left"><div style="display:flex; align-items:center; margin-bottom:6px;"><span class="member-badge" style="background:#bfdbfe; color:#2563eb;"><i class="fa-regular fa-calendar"></i> ${showDate}</span><strong style="font-size:15px;">M: ${record.morning}L | E: ${record.evening}L</strong></div><div style="font-size:12px; color:#64748b; font-weight:600;">Rate: ₹${record.rate}/L | Total: ${totalDayLiter}L</div></div><div class="list-right"><span style="font-weight:800; color:#2563eb; font-size:19px; margin-right:5px;">₹${dayCost}</span><button class="action-btn edit" onclick="editDudh(${index})"><i class="fa-solid fa-pen"></i></button><button class="action-btn delete" onclick="deleteDudh(${index})"><i class="fa-solid fa-trash"></i></button></div>`; list.appendChild(li); }); document.getElementById('dudh-total-liter').innerText = totalLiter.toFixed(2); document.getElementById('dudh-total-bill').innerText = `₹${Math.round(totalBill)}`; renderIcons(); }
function addDudh() { const dDate = document.getElementById('dudh-date').value || todayDateString; const rate = parseFloat(document.getElementById('dudh-rate').value); const morn = parseFloat(document.getElementById('dudh-morning').value) || 0; const eve = parseFloat(document.getElementById('dudh-evening').value) || 0; if (!dDate || isNaN(rate) || (morn === 0 && eve === 0)) return Swal.fire('Error', 'Sahi details daaliye!', 'error'); if(editDudhIndex === -1) { dudhRecords.push({ date: dDate, rate: rate, morning: morn, evening: eve }); playSound('success'); } else { dudhRecords[editDudhIndex] = { date: dDate, rate: rate, morning: morn, evening: eve }; editDudhIndex = -1; document.getElementById('btn-add-dudh').innerHTML = '<i class="fa-solid fa-plus"></i> Add Record'; } saveToCloud(); updateDudhUI(); document.getElementById('dudh-morning').value = ''; document.getElementById('dudh-evening').value = ''; }
function editDudh(index) { const item = dudhRecords[index]; document.getElementById('dudh-date').value = item.date; document.getElementById('dudh-rate').value = item.rate; document.getElementById('dudh-morning').value = item.morning; document.getElementById('dudh-evening').value = item.evening; editDudhIndex = index; document.getElementById('btn-add-dudh').innerHTML = '<i class="fa-solid fa-pen"></i> Update'; window.scrollTo({ top: 0, behavior: 'smooth' }); playSound('click'); renderIcons(); }
...
function printReport() { 
    ...
    if(exps.length===0) return Swal.fire('Empty', 'Is mahine ka koi data nahi hai!', 'info'); 
    
    let totalAmt = 0; let trs = '';
    exps.sort((a,b)=>new Date(b.date)-new Date(a.date)).forEach(e => {
        let cleanCat = e.category ? String(e.category).trim() : '';
        let cleanDesc = e.description ? String(e.description).trim() : '';
        let cleanMem = e.member ? String(e.member).trim() : '';
    ...
    if (!printFrame) { printFrame = document.createElement('iframe'); printFrame.id = 'print-frame'; printFrame.style.position = 'absolute'; printFrame.style.top = '-10000px'; document.body.appendChild(printFrame); }
    
    const doc = printFrame.contentWindow.document;
    doc.open(); doc.write('<html><head><title>Print Report</title></head><body>' + html + '</body></html>'); doc.close();
    setTimeout(() => { printFrame.contentWindow.focus(); printFrame.contentWindow.print(); }, 500); playSound('click'); 
}
...
async function shareReport() {
    ...
    if(dataToExport.length === 0) return Swal.fire('Empty!', 'Koi record nahi hai.', 'info');
    
    const { jsPDF } = window.jspdf; const doc = new jsPDF(); 
    
    let catTotals = {};
    dataToExport.forEach(e => { catTotals[e.category] = (catTotals[e.category] || 0) + parseFloat(e.amount); });
    
    const tempCanvas = document.createElement('canvas'); tempCanvas.width = 400; tempCanvas.height = 400; document.body.appendChild(tempCanvas);
    const tempCtx = tempCanvas.getContext('2d');
    new Chart(tempCtx, { type: 'pie', data: { labels: Object.keys(catTotals), datasets: [{ data: Object.values(catTotals), backgroundColor: ['#2563eb', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6', '#64748b'] }] }, options: { animation: false, plugins: { legend: { labels: { color: '#000', font: {size: 14} } } } } });
    ...
    setTimeout(() => {
        ...
        doc.setFontSize(11); 
        doc.text(`Total Income: Rs ${monthlyIncome}`, 14, 55); 
        doc.text(`Total Expense: Rs ${totalAmount}`, 14, 62); 
        doc.text(`Net Savings: Rs ${monthlyIncome - totalAmount}`, 14, 69);
        
        let maxCat = ""; let maxAmt = 0;
        for(let c in catTotals) { if(catTotals[c] > maxAmt) { maxAmt = catTotals[c]; maxCat = c; } }
        
        doc.setFillColor(239, 246, 255); doc.rect(100, 48, 95, 25, 'F');
        doc.setTextColor(29, 78, 216); doc.setFontSize(10);
        doc.text(`AI Insights:`, 105, 55);
        doc.text(`- Biggest expense: ${maxCat} (Rs ${maxAmt})`, 105, 62);
        doc.text(`- Savings Health: ${monthlyIncome > totalAmount ? 'Good' : 'Warning'}`, 105, 69);

        doc.addImage(chartImage, 'PNG', 75, 80, 60, 60);
        ...
        doc.autoTable({ head: [tableColumn], body: tableRows, startY: 150, theme: 'striped', headStyles: { fillColor: [16, 185, 129] }, styles: { fontSize: 10 }, foot: [["", "", "", "Total :", `Rs ${totalAmount}`]], footStyles: { fillColor: [239, 68, 68], textColor: [255,255,255] } });
        
        const pageCount = doc.internal.getNumberOfPages();
        for(let i = 1; i <= pageCount; i++) {
            doc.setPage(i);
            doc.setFontSize(10); doc.setTextColor(100);
            doc.text(`Generated by GharManager AI  |  Page ${i} of ${pageCount}`, doc.internal.pageSize.width / 2, doc.internal.pageSize.height - 10, { align: 'center' });
        }
        ...
        Swal.fire({ title: 'PDF Ready!', text: 'PDF ban gayi hai, kya karna chahte ho?', icon: 'success', showCancelButton: true, confirmButtonText: '<i class="fa-solid fa-download"></i> Download', cancelButtonText: '<i class="fa-solid fa-share-nodes"></i> Share', confirmButtonColor: '#10b981', cancelButtonColor: '#3b82f6' }).then(async (result) => {
...
function openAIBottomSheet() {
    Swal.fire({
        title: '<i class="fa-solid fa-robot"></i> AI Assistant',
        html: `<div style="display:flex; flex-direction:column; gap:10px;"><button onclick="Swal.close(); askFinanceAI();" style="background:#6366f1; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold; cursor:pointer;"><i class="fa-solid fa-chart-pie"></i> AI Spending Report</button><button onclick="Swal.close(); startVoice();" style="background:#8b5cf6; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold; cursor:pointer;"><i class="fa-solid fa-microphone"></i> Voice Kharcha Add</button></div>`,
...
function askFinanceAI() {
    ...
    for (let cat in currCat) {
        ...
        if(pastCat[cat] && currCat[cat] > pastCat[cat]) {
            ...
            if(percentInc > 15) alertMsg = `<i class="fa-solid fa-triangle-exclamation"></i> <b>Unusual Spending:</b> Tumhara <b>${cat}</b> ka kharcha pichle mahine se <b>${percentInc}% badh gaya hai!</b> Control karo!`;
        }
    }
    let totalExpMonth = currentData.reduce((sum, item) => sum + parseFloat(item.amount), 0);
    let savings = monthlyIncome - totalExpMonth;
    let savingsMsg = savings > 0 ? `<span style="color:#16a34a; font-weight:bold;"><i class="fa-solid fa-check"></i> Badiya! Tumne is mahine ₹${savings} bacha liye hain.</span>` : `<span style="color:#e74c3c; font-weight:bold;"><i class="fa-solid fa-triangle-exclamation"></i> Alert! Tumhari kamai se zyada kharcha (₹${Math.abs(savings)} extra) ho raha hai!</span>`;
    Swal.fire({ title: '<i class="fa-solid fa-robot"></i> Real AI Insights', html: `<div style="text-align: left; font-size: 14px; line-height: 1.6;"><p><i class="fa-solid fa-chart-simple"></i> <b>Top Kharcha:</b> Sabse zyada paisa <b>${maxCat || 'kisi cheez'} (₹${maxAmt})</b> mein gaya hai.</p><hr style="margin: 10px 0; border: 0.5px dashed #cbd5e1;"><p><i class="fa-solid fa-lightbulb"></i> <b>Trend Alert:</b> ${alertMsg}</p><hr style="margin: 10px 0; border: 0.5px dashed #cbd5e1;"><p><i class="fa-solid fa-piggy-bank"></i> <b>Savings:</b> ${savingsMsg}</p></div>`, icon: 'info', confirmButtonText: 'Thanks AI!', confirmButtonColor: '#6366f1' });
    renderIcons();
}
...
function startVoice() { 
    ...
    try {
        ...
        recognition.lang = 'hi-IN'; const btn = document.getElementById('mic-btn'); btn.innerHTML = '<i class="fa-solid fa-stop"></i>'; renderIcons();
        recognition.onresult = (event) => { 
            ...
            btn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; playSound('click'); 
        }; 
        recognition.onerror = (e) => { btn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; console.error(e); if(e.error === 'not-allowed') { Swal.fire('Permission Denied', 'Browser me Microphone ki permission on karein!', 'error'); } else { Swal.fire('Error', 'Awaz clear nahi aayi!', 'error'); } renderIcons(); }; 
        recognition.onend = () => { btn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; renderIcons(); }; recognition.start(); playSound('click');
    } catch(e) { document.getElementById('mic-btn').innerHTML = '<i class="fa-solid fa-microphone"></i>'; console.error(e); Swal.fire('Error', 'Mic start nahi hua. ' + e.message, 'error'); renderIcons(); }
}
