...
// PREMIUM PDF EXPORT WITH EMOJI REMOVER
async function shareReport() {
    ...
    // Temporary canvas for PDF Chart
    ...
    setTimeout(() => {
        ...
        // AI Insights Box in PDF
        doc.setFillColor(239, 246, 255); doc.rect(100, 48, 95, 25, 'F'); // Blue Box
        doc.setTextColor(29, 78, 216); doc.setFontSize(10);
        doc.text(`🤖 AI Insights:`, 105, 55);
        doc.text(`• Biggest expense: ${maxCat} (Rs ${maxAmt})`, 105, 62);
        doc.text(`• Savings Health: ${monthlyIncome > totalAmount ? 'Good 👍' : 'Warning ⚠️'}`, 105, 69);
...
// 🛡️ THE REAL GEMINI AI API (SMART FIX & 1.5 PRO)
async function sendToRealAI() {
    ...
    chatBox.innerHTML += `<div style="margin:5px 0; text-align:right;"><span style="background:#3b82f6; color:white; padding:8px 12px; border-radius:16px 16px 0 16px; display:inline-block; max-width:80%; word-wrap:break-word;">${userText}</span></div>`;
    ...
    let expensesList = monthData.map(e => `${e.category}:${e.amount}`).join(', ');
    
    let systemPrompt = `You are GharManager AI, a smart Indian family finance assistant.
    Help users track expenses, save money, understand spending, and give financial insights.
    Current Data: Total Expense = ₹${totalExp}, Income = ₹${monthlyIncome}. Expenses list: ${expensesList}.
    Always answer in simple Roman Hindi (Hinglish).
    User Query: ${userText}`;

    let loadingId = 'load-' + Date.now();
    chatBox.innerHTML += `<div id="${loadingId}" style="margin:5px 0; text-align:left;"><span style="background:var(--paper-bg); color:var(--text-main); border:1px solid #cbd5e1; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block;">AI is thinking... ⏳</span></div>`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 429 || errMsg.includes('Quota exceeded')) {
                errMsg = "Google AI Free Limit Khatam! 🛑 Thodi der baad try karein ya Nayi API Key banayein.";
            }
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; font-size:12px; font-weight:bold;"><b>API Error:</b> ${errMsg}</span></div>`;
            chatBox.scrollTop = chatBox.scrollHeight;
            return;
        }
        ...
        if (data.candidates && data.candidates[0].content) {
            ...
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left; animation: fadeIn 0.3s;"><span style="background:var(--paper-bg); color:var(--text-main); border:1px solid #cbd5e1; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; max-width:90%; word-wrap:break-word; line-height:1.4;">${formatText}</span></div>`;
        } else {
            chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block;">AI ne koi jawab nahi diya.</span></div>`;
        }
        
        chatBox.scrollTop = chatBox.scrollHeight; playSound('success');
    } catch(e) {
        if(document.getElementById(loadingId)) document.getElementById(loadingId).remove();
        chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:8px 12px; border-radius:16px 16px 16px 0; display:inline-block; font-size:12px; font-weight:bold;">Internet ya Network Error! Check console.</span></div>`;
...
function openAIBottomSheet() {
    Swal.fire({
        ...
        html: `<div style="display:flex; flex-direction:column; gap:10px;">
                <button onclick="Swal.close(); askFinanceAI();" class="expense-btn" style="background:#6366f1; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold;">📊 Get AI Spending Report</button>
                <button onclick="Swal.close(); startVoice();" class="expense-btn" style="background:#8b5cf6; color:white; border:none; padding:12px; border-radius:12px; font-weight:bold;">🎤 Add Kharcha by Voice</button>
                <hr style="border: 0.5px dashed #cbd5e1; margin: 5px 0;">
                <p style="font-size:12px; font-weight:bold; color:var(--ink-blue); text-align:left; margin:0;">💬 Chat with AI (Gemini 1.5 Pro)</p>
                <div id="ai-chat-box" style="background:var(--line-color); height:200px; border-radius:12px; padding:10px; font-size:13px; overflow-y:auto; text-align:left; margin-bottom:5px; scroll-behavior: smooth;">
                    Hello! Main GharManager AI hu. Mujhse apne kharcho ke baare me poocho...
                </div>
                <div style="display:flex; gap:8px; align-items:center;">
                    <input type="text" id="ai-user-input" placeholder="Type here..." style="flex:1; width:100%; min-height:50px; padding:14px; border-radius:16px; border:none; outline:none; font-size:14px; background:#f3f4f6; color:#0f172a; box-sizing: border-box;">
                    <button onclick="sendToRealAI()" style="background:#10b981; color:white; border:none; padding:0 20px; border-radius:16px; font-weight:bold; cursor:pointer; height:50px; display:flex; align-items:center; justify-content:center;">Send</button>
                </div>
               </div>`,
        position: 'bottom', showConfirmButton: false, showCloseButton: true, customClass: { popup: 'animate__animated animate__slideInUp', container: 'bottom-sheet-container' }, width: '100%', background: 'var(--bg-color)', color: 'var(--text-main)'
    }); 
    playSound('click');
}
