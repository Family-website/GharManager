...
function addExpense() {
    try {
        ...
        const newRecord = {
            ...
            amount: amt,
            date: date,
            receipt: typeof currentReceiptUrl !== 'undefined' ? currentReceiptUrl : "",
            gps: typeof currentGPSLocation !== 'undefined' ? currentGPSLocation : null
        };
...
// 🛡️ 11. THE REAL GEMINI AI API (BUG FIX 3)
async function sendToRealAI() {
    // 🔑 YAHAN APNI ASLI API KEY HAI
    ...
    let context = `User ka is mahine ka total kharcha ₹${totalExp} hai aur kamai ₹${monthlyIncome} hai. Kharcho ki list: ${expensesList}. User ke sawal ka chhota aur point-to-point jawab Roman Hindi (Hinglish) me do. Markdown formatting jaise asterisks (**) ka use mat karna. Question: ${userText}`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            // Agar fail hua, toh yahan dikhayega asli Google API Error
        ...
        chatBox.innerHTML += `<div style="margin:5px 0; text-align:left;"><span style="background:#fee2e2; color:#991b1b; padding:6px 10px; border-radius:12px; display:inline-block; font-size:11px;">Internet ya Network Error! Check console.</span></div>`;
...
function openAIBottomSheet() {
    Swal.fire({
        ...
        html: `<div style="display:flex; flex-direction:column; gap:10px;">
                ...
                <p style="font-size:12px; font-weight:bold; color:var(--ink-blue); text-align:left; margin:0;">💬 Chat with AI (Gemini)</p>
                ...
                <div style="display:flex; gap:8px; align-items:center;">
                    ...
                    <button onclick="sendToRealAI()" class="expense-btn" style="background:#10b981; color:white; border:none; padding:10px 15px; border-radius:8px; font-weight:bold; cursor:pointer;">Send</button>
                </div>
               </div>`,
