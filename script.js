...
function playSound(type) { if(navigator.vibrate) navigator.vibrate(type === 'success' ? [30, 50, 30] : 30); if(!isSoundEnabled) return; try { if(type === 'click') document.getElementById('sound-click').play(); if(type === 'success') document.getElementById('sound-success').play(); } catch(e) {} }
...
document.addEventListener('touchend', e => {
    ...
    if(pEndY > pStartY + 150 && window.scrollY === 0 && currentUser) {
        ...
        pRef.style.display = 'block'; pRef.innerText = '🔄 Syncing...';
...
function renderCalendar(exps, fm) {
    ...
    let dt = {}; exps.forEach(e => { let day = parseInt(e.date.split('-')[2]); dt[day] = (dt[day] || 0) + e.amount; });
    for(let i=1; i<=dim; i++) {
...
function updateHisabUI() {
    ...
    if(filteredExpenses.length === 0) {
        ...
        filteredExpenses.forEach((item) => {
            if(isDateSorted && item.date !== currentDateHeader) { currentDateHeader = item.date; const parts = currentDateHeader.split('-'); const dateObj = new Date(parts[0], parts[1] - 1, parts[2]); const showDate = `${dateObj.getDate()} ${['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][dateObj.getMonth()]}`; const dateHeader = document.createElement('div'); dateHeader.className = 'date-header'; dateHeader.style.fontWeight = 'bold'; dateHeader.style.color = 'var(--ink-blue)'; dateHeader.style.margin = '10px 0 5px 0'; dateHeader.innerText = `📅 ${showDate}`; list.appendChild(dateHeader); }
...
function renderTrendChartProfile(exps) { const ctx = document.getElementById('trendChart'); if(!ctx) return; if(trendChartInstance) trendChartInstance.destroy(); let dt = {}; exps.forEach(e => { let d = e.date.split('-')[2]; dt[d] = (dt[d] || 0) + e.amount; }); const labels = Object.keys(dt).sort((a,b) => parseInt(a) - parseInt(b)); const data = labels.map(d => dt[d]); const tc = isDarkMode ? '#fff' : '#333'; trendChartInstance = new Chart(ctx.getContext('2d'), { type: 'bar', data: { labels: labels.map(l => l + ' Date'), datasets: [{ label: 'Daily Spend (₹)', data: data, backgroundColor: '#8b5cf6', borderRadius: 6 }] }, options: { responsive: true, plugins: { legend: { labels: { color: tc } } }, scales: { x: { ticks: { color: tc } }, y: { ticks: { color: tc } } } } }); }
