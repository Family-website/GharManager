...
window.addEventListener('online', () => { isOffline = false; document.getElementById('offline-banner').style.display = 'none'; if(typeof GharDB !== 'undefined') GharDB.processSyncQueue().then(() => BATCH_ENGINE.flush()); else syncOldLocalData(); playSound('success'); });
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        await initGharDB(); loadCloudData(user.uid); await syncOldLocalData();
...
// ✅ loadCloudData, saveToCloud, syncOldLocalData — moved to db-engine.js v2.0
...
// ✅ backupData, restoreData — moved to db-engine.js v2.0 (AES-256-GCM encrypted)
