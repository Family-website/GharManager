...
// ==========================================
// HELPER: FORCE SVG RENDER FOR SPCK
// ==========================================
function renderIcons() {
    try {
        if(window.FontAwesome) { FontAwesome.dom.i2svg(); }
    } catch(e) { console.warn("SVG Engine skipped temporarily"); }
}
...
function checkRole(name) { 
    ...
    if(n === 'me' || n.includes('papa')) { 
        userRole = 'Admin'; 
        let b = document.getElementById('role-badge'); 
        if(b){ 
            b.innerHTML = '<i class="fa-solid fa-shield-halved"></i> Admin'; 
        ...
        let b = document.getElementById('role-badge'); 
        if(b){ 
            b.innerHTML = '<i class="fa-solid fa-user"></i> Member'; 
...
function checkSmartReminders() {
    ...
    if(!rs) {
        ...
        if(alerts.length > 0) { Swal.fire('Recharge Alert!', `Bhai, "${alerts.join(', ')}" ka recharge 2 din me khatam hone wala hai!`, 'warning'); }
        else {
            ...
            else if(todayDate >= 1 && todayDate <= 5) Swal.fire('Monthly Reminder', 'Kiraya/bills baaki hain toh clear kar lo!', 'info');
        }
...
function openProfile() {
    try {
        ...
        const lvlBadge = document.getElementById('profile-level-badge'); if(lvlBadge) { let level = Math.floor((parseInt(userXP) || 0) / 100) + 1; let title = level < 3 ? "Beginner" : level < 6 ? "Pro Saver" : "Finance Ninja"; lvlBadge.innerHTML = `<i class="fa-solid fa-shield"></i> Level ${level} | ${title} (XP: ${userXP})`; }
        ...
        let netWorth = (parseFloat(monthlyIncome) || 0) + totalInvestments - totalExpAllTime - totalLoanLeft; let nwEl = document.getElementById('profile-net-worth'); if(nwEl) { if(netWorth >= 0) { nwEl.style.color = '#10b981'; nwEl.innerHTML = `₹${Math.round(netWorth)} <i class="fa-solid fa-arrow-trend-up"></i>`; } else { nwEl.style.color = '#ef4444'; nwEl.innerHTML = `₹${Math.round(netWorth)} <i class="fa-solid fa-arrow-trend-down"></i>`; } }
        ...
        setTimeout(() => {
            ...
            } catch(e) { console.log("Chart render skipped"); }
        }, 300);
...
function convertToUSD() {
    ...
    if(!nwEl) return;
    let inrValue = parseFloat(nwEl.innerText.replace(/[^0-9.-]+/g,""));
    if(isNaN(inrValue)) return Swal.fire('Error', 'Net worth calculate nahi ho paya!', 'error');
    
    let usdValue = (inrValue / 83.5).toFixed(2);
    
    Swal.fire({
        title: 'Global Net Worth',
        html: `<h1 style="color: #10b981; font-size: 40px; margin: 10px 0;">$${usdValue} USD</h1><p style="font-size: 12px; color: #64748b;">(Approx 83.5₹ per $)</p>`,
        icon: 'success',
...
function inviteFamily() {
    ...
    if (navigator.share) {
        navigator.share({
            title: 'GharManager Pro',
            text: 'Ghar ka hisaab ab ek jagah! Mera invite accept karo aur GharManager par aao:',
            url: window.location.href
        }).catch(console.error);
    } else {
        Swal.fire('Link Ready!', 'Browser share support nahi karta. URL copy karke family ko WhatsApp kar do.', 'info');
    }
}
...
function openAIBottomSheet() {
    Swal.fire({
        title: '<i class="fa-solid fa-wand-magic-sparkles"></i> AI Assistant',
...
function startVoice() { 
    ...
    try {
        ...
        recognition.onresult = (event) => { 
            ...
            btn.innerHTML = '<i class="fa-solid fa-microphone"></i>'; playSound('click'); renderIcons();
        }; 
