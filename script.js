...
// ==========================================
// 🔐 2. LOGIN & BIOMETRIC (Phase 15 Update)
// ==========================================
...
let appLang = localStorage.getItem('appLang') || 'Hinglish'; // 🌐 Lang State
...
// ==========================================
// 🏆 3.8. XP, CHALLENGE & TO-DO
// ==========================================
function updateProfileName() { const currentName = customDisplayName || (currentUser ? currentUser.email.split("@")[0] : "User"); Swal.fire({ title: 'Apna Naam Likhein', input: 'text', inputValue: currentName, showCancelButton: true, confirmButtonText: 'Save Karein', confirmButtonColor: '#2563eb' }).then((result) => { if (result.isConfirmed) { customDisplayName = result.value.trim(); saveToCloud(); updateGreetingName(); Swal.fire('Saved!', 'Aapka naam update ho gaya hai.', 'success'); } }); }
function updateGreetingName() { if (!currentUser) return; const finalName = customDisplayName || currentUser.email.split("@")[0]; const NameFormatted = finalName.charAt(0).toUpperCase() + finalName.slice(1); const firstLetter = finalName.charAt(0).toUpperCase(); const profileNameEl = document.getElementById('profile-name'); const avatarEl = document.getElementById('user-avatar'); const largeAvatarEl = document.getElementById('profile-avatar-large'); if(profileNameEl) profileNameEl.innerText = NameFormatted; if(avatarEl) avatarEl.innerText = firstLetter; if(largeAvatarEl) largeAvatarEl.innerText = firstLetter; }
...
// 🌐 MULTI-LANGUAGE TOGGLE (Phase 15)
const dict = {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if(descInput) { descInput.addEventListener('input', function(e) { let val = e.target.value.toLowerCase(); let cat = document.getElementById('expense-category'); if(val.includes('dawa') || val.includes('doctor') || val.includes('hospital')) cat.value = 'Medical'; else if(val.includes('petrol') || val.includes('diesel') || val.includes('bike') || val.includes('gaadi')) cat.value = 'Petrol'; else if(val.includes('sabji') || val.includes('ration') || val.includes('chawal') || val.includes('tel')) cat.value = 'Ration'; else if(val.includes('recharge') || val.includes('bill') || val.includes('bijli') || val.includes('wifi') || val.includes('netflix')) cat.value = 'Bills'; else if(val.includes('kapde') || val.includes('shirt') || val.includes('shoes') || val.includes('shopping')) cat.value = 'Shopping'; }); }
});
...
// 🏦 NAYA: BANK CSV IMPORT LOGIC (Phase 15)
function importBankCSV() {
    ...
    reader.onload = function(e) {
        ...
        for(let i=1; i<lines.length; i++) { // Skip header
            ...
            if(cols.length >= 3) {
                // Assuming format: Date, Narration/Description, Amount (or Withdrawal)
                let dateVal = cols[0].trim(); let descVal = cols[1].trim(); let amtVal = parseFloat(cols[2] || cols[3] || 0); // basic parser
                if(!isNaN(amtVal) && amtVal > 0 && descVal.length > 2) {
                    let fDate = todayDateString; // default
                    try { let d = new Date(dateVal); if(!isNaN(d)) fDate = d.toISOString().split('T')[0]; } catch(err){}
...
function backupData() { 
    ...
    const encryptedData = btoa(unescape(encodeURIComponent(JSON.stringify(dataToBackup)))); // Secure Base64
...
function openProfile() {
    ...
    let totalInvestments = investments.reduce((sum, item) => sum + item.amount, 0); let totalLoanLeft = activeLoans.reduce((sum, item) => sum + (item.principal - (item.principal * (item.monthsPaid/item.time))), 0);
    let netWorth = (monthlyIncome + totalInvestments) - (totalExpAllTime + totalLoanLeft); let nwEl = document.getElementById('profile-net-worth');
    ...
    let score = 50; if(monthlyIncome > 0) { let savePercent = ((monthlyIncome - monthExpenses) / monthlyIncome) * 100; if(savePercent >= 20) score = 95; else if(savePercent >= 10) score = 75; else if(savePercent >= 0) score = 60; else score = 30; }
    ...
    if(document.getElementById('goal-name')) { document.getElementById('goal-name').innerText = dreamGoal.name; document.getElementById('goal-target').innerText = dreamGoal.target; let currentSavings = monthlyIncome > monthExpenses ? monthlyIncome - monthExpenses : 0; document.getElementById('goal-saved').innerText = currentSavings; let percent = dreamGoal.target > 0 ? (currentSavings / dreamGoal.target) * 100 : 0; if(percent > 100) percent = 100; document.getElementById('goal-bar').style.width = `${percent}%`; }

    let monthData = familyExpenses.filter(item => item.date && item.date.startsWith(filterMonth)); let memTotals = {}; monthData.forEach(exp => { memTotals[exp.member] = (memTotals[exp.member] || 0) + exp.amount; });
...
async function convertToUSD() {
    ...
    try {
        ...
        let totalInvestments = investments.reduce((sum, item) => sum + item.amount, 0); let totalLoanLeft = activeLoans.reduce((sum, item) => sum + (item.principal - (item.principal * (item.monthsPaid/item.time))), 0); let totalExpAllTime = familyExpenses.reduce((sum, item) => sum + item.amount, 0);
...
// ==========================================
// 🤖 11. LOCAL AI FINANCIAL ASSISTANT & SMART VOICE
// ==========================================
function startVoice() { 
