...
// ============================================================
// 📱 PWA UPGRADE v3.0 — Install Prompt + Push + Background Sync
// ============================================================
...
if ('serviceWorker' in navigator) {
    ...
    navigator.serviceWorker.addEventListener('message', (event) => {
        ...
        if (type === 'SYNC_COMPLETE' || type === 'EXPENSE_SYNC') {
            Swal.fire({ toast: true, position: 'top', icon: 'success', title: message || 'Data sync ho gaya! ✅', timer: 3000, showConfirmButton: false });
        }
    });
}

// ── 2. INSTALL PROMPT (Custom Banner) ──────────────────────
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    if (!localStorage.getItem('pwa_installed') && !_installBannerShown) {
...
window.addEventListener('appinstalled', () => {
    ...
    hideInstallBanner();
    Swal.fire({ toast: true, position: 'top', icon: 'success', title: '🎉 GharManager install ho gaya!', text: 'Home screen pe shortcut mil gaya!', timer: 4000, showConfirmButton: false });
...
function showInstallBanner() {
    ...
    if (banner) {
        ...
        banner.style.animation = 'slideUpBanner 0.4s cubic-bezier(0.34,1.56,0.64,1)';
    }
}
...
async function triggerInstall() {
    if (!_installPromptEvent) {
        ...
        Swal.fire({
            title: '📱 Install Karein',
            html: `<div style="text-align:left;font-size:14px;font-weight:600;color:var(--text-main);line-height:2;">
                <p style="font-weight:800;margin-bottom:10px;color:var(--ink-blue);">iOS (Safari) pe install karne ke liye:</p>
                <p>1️⃣ Neeche <b>Share</b> button dabayein <span style="font-size:18px;">⬆️</span></p>
                <p>2️⃣ <b>"Add to Home Screen"</b> chunein</p>
                <p>3️⃣ <b>Add</b> press karein — Ho gaya! 🎉</p>
            </div>`,
            confirmButtonText: 'Samajh Gaya ✅', confirmButtonColor: '#2563eb',
            background: 'var(--paper-bg)', color: 'var(--text-main)'
        });
        return;
    }
...
function dismissInstallBanner() {
    ...
    setTimeout(() => { localStorage.removeItem('pwa_install_dismissed'); }, 3 * 24 * 60 * 60 * 1000);
}
...
async function requestPushPermission() {
    if (!('Notification' in window)) {
        return Swal.fire('Sorry', 'Aapka browser push notifications support nahi karta.', 'warning');
    }
    ...
    if (Notification.permission === 'denied') {
        return Swal.fire({
            ...
            html: `<p style="font-size:14px;font-weight:600;">Browser settings mein notifications allow karein:<br><br>
            <b>Chrome:</b> Address bar ke lock icon → Notifications → Allow<br>
            <b>Firefox:</b> Address bar ka shield icon → Allow</p>`,
            icon: 'info', confirmButtonColor: '#2563eb'
        });
    }

    // Ask permission with context first
    const result = await Swal.fire({
        title: '🔔 Notifications On Karein?',
        html: `<p style="font-size:14px;font-weight:600;color:var(--text-muted);line-height:1.7;">
            GharManager aapko remind karega:<br>
            📊 Monthly report ready hone par<br>
            ⚠️ Budget limit cross hone par<br>
            🥛 Dudh record bharne ka reminder<br>
            ☁️ Data sync complete hone par
        </p>`,
        icon: 'info', showCancelButton: true,
        confirmButtonText: '🔔 Allow Karein', cancelButtonText: 'Baad Mein',
    ...
    if (permission === 'granted') {
        await subscribeToPush();
        Swal.fire({ toast: true, position: 'top', icon: 'success', title: '🔔 Notifications on ho gaye!', timer: 2500, showConfirmButton: false });
    }
}
...
async function subscribeToPush() {
    try {
        ...
        // Skip if no real VAPID key configured
        if (VAPID_PUBLIC_KEY === 'YOUR_VAPID_PUBLIC_KEY_HERE') {
            console.log('[PWA] Push ready — set VAPID key to activate server push');
            localStorage.setItem('push_enabled', 'true');
            return;
        }
        ...
        localStorage.setItem('push_enabled', 'true');
        console.log('[PWA] Push subscription:', JSON.stringify(sub));
        // TODO: Send `sub` to your server / Firebase to store for sending pushes
        return sub;
...
async function sendLocalNotification(title, body, options = {}) {
    ...
    reg.showNotification(title, {
        ...
        vibrate: [200, 100, 200], tag: options.tag || 'gm-local',
        data: { url: './' }, ...options
    });
}

// ── Scheduled local reminders (called on app load) ──────────
function setupSmartNotifications() {
    if (Notification.permission !== 'granted') return;
    const now = new Date();
    const hour = now.getHours();

    // Evening reminder to log dudh (7–8 PM)
    if (hour >= 19 && hour < 20 && !localStorage.getItem(`dudh_notif_${now.toDateString()}`)) {
        sendLocalNotification('🥛 Dudh Record Yaad Hai?', 'Aaj ka shaam ka dudh log karna baaki hai!', { tag: 'dudh-reminder' });
        localStorage.setItem(`dudh_notif_${now.toDateString()}`, '1');
    }

    // Budget warning
    if (typeof budgetLimit !== 'undefined' && typeof familyExpenses !== 'undefined') {
        const thisMonth = now.toISOString().slice(0, 7);
        const spent = familyExpenses.filter(e => e.date?.startsWith(thisMonth)).reduce((s, e) => s + parseFloat(e.amount || 0), 0);
        const pct = budgetLimit > 0 ? (spent / budgetLimit) * 100 : 0;
        if (pct >= 90 && !localStorage.getItem(`budget_warn_${thisMonth}`)) {
            sendLocalNotification('⚠️ Budget Alert!', `Aapka ${Math.round(pct)}% budget use ho gaya. Kharcha control karein!`, { tag: 'budget-warn' });
            localStorage.setItem(`budget_warn_${thisMonth}`, '1');
        }
    }
}

// ── 4. BACKGROUND SYNC — Queue offline expenses ────────────
const SYNC_QUEUE_KEY = 'gm_offline_queue';

function queueOfflineExpense(expense) {
    const queue = JSON.parse(localStorage.getItem(SYNC_QUEUE_KEY) || '[]');
    queue.push({ ...expense, _queued: Date.now(), _id: Date.now() + Math.random() });
    localStorage.setItem(SYNC_QUEUE_KEY, JSON.stringify(queue));
    console.log(`[Sync] Queued offline expense: ${expense.name}`);
    triggerBackgroundSync();
}

async function triggerBackgroundSync() {
    if (!window._swSyncSupported || !window._swReg) return;
    try {
        await window._swReg.sync.register('gm-expense-sync');
        console.log('[Sync] Background sync registered');
    } catch (err) {
        console.warn('[Sync] Could not register sync:', err);
        // Fallback: try syncing immediately if online
...
async function syncOfflineQueue() {
    ...
    for (const expense of queue) {
        try {
            ...
            if (typeof db !== 'undefined' && typeof currentUser !== 'undefined' && currentUser) {
                const { _queued, _id, ...cleanExpense } = expense;
                await db.collection('users').doc(currentUser.uid)
                    .collection('expenses').add(cleanExpense);
                synced++;
            }
        } catch (err) {
    ...
    if (synced > 0) {
        ...
        Swal.fire({ toast: true, position: 'top', icon: 'success', title: `☁️ ${synced} expense${synced > 1 ? 's' : ''} sync ho gaya!`, timer: 3000, showConfirmButton: false });
    }
}
...
window.addEventListener('load', () => {
    ...
    // Setup smart notifications after short delay
    setTimeout(setupSmartNotifications, 5000);
    // Sync offline queue if any pending
