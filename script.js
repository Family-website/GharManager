...
function updateHisabUI() {
    ...
    if(filteredExpenses.length === 0) {
        ...
        // Group items by date first for daily totals
        const dateGroups = {};
        filteredExpenses.forEach(item => {
            if (!dateGroups[item.date]) dateGroups[item.date] = { total: 0 };
            dateGroups[item.date].total += parseFloat(item.amount);
        });
        // Map for human-readable date
        ...
        filteredExpenses.forEach((item) => {
            if(isDateSorted && item.date !== currentDateHeader) {
                ...
                let labelText = `${parseInt(parts[2])} ${months[dateObj.getMonth()]}`;
                if(currentDateHeader === todayStr) labelText = 'AAJ';
                else if(currentDateHeader === yesterdayStr) labelText = 'KAL';
                const dayTotal = dateGroups[currentDateHeader]?.total || 0;
                const dateHeader = document.createElement('div');
                dateHeader.className = 'hx-date-group-header';
                dateHeader.innerHTML = `<span class="hx-dg-label">${labelText}</span><span class="hx-dg-total">₹${Math.round(dayTotal).toLocaleString('en-IN')}</span>`;
            ...
            // Extract time from date or use item.time if available
            ...
            li.innerHTML = `<div class="list-left" style="pointer-events:none;"><div style="display:flex;align-items:center;gap:12px;"><span style="background:${catMeta.bg};font-size:22px;width:46px;height:46px;border-radius:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${catMeta.icon}</span><div style="flex:1;min-width:0;"><strong style="font-size:15px;line-height:1.3;color:var(--text-main);font-family:'DM Sans',sans-serif;font-weight:700;display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${item.description}</strong><div style="display:flex;align-items:center;gap:6px;margin-top:4px;"><span style="background:${catMeta.bg};color:${catMeta.color};padding:3px 10px;border-radius:999px;font-size:10px;font-weight:800;">${item.category||'Other'}</span><span style="font-size:10px;color:var(--text-faint);font-weight:600;">🙋 ${item.member}</span></div></div></div></div><div class="list-right" style="flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0;pointer-events:none;">${receiptHTML}<span style="font-weight:900;color:${catMeta.color};font-size:20px;font-family:'Sora',sans-serif;letter-spacing:-0.5px;line-height:1;">₹${parseFloat(item.amount).toLocaleString('en-IN')}</span>${itemTime ? `<span style="font-size:10px;color:var(--text-faint);font-weight:600;">${itemTime}</span>` : item.date ? `<span style="font-size:10px;color:var(--text-faint);font-weight:600;">${item.date.slice(8)} ${months[parseInt(item.date.slice(5,7))-1]}</span>` : ''}<button class="action-btn delete" onclick="event.stopPropagation(); deleteExpense(${originalIndex})" style="width:26px;height:26px;font-size:12px;margin-top:2px;">🗑️</button></div>`;
    ...
    let budgetPercent = Math.min((totalExpense / budgetLimit) * 100, 100).toFixed(1);
    // Update both old budget-bar id and new hero fill (same element in new layout)
    const bar = document.getElementById('budget-bar'); 
    if(bar) { bar.style.width = `${budgetPercent}%`; if(parseFloat(budgetPercent) < 50) bar.style.background = 'linear-gradient(90deg,#22D3EE,#10B981)'; else if(parseFloat(budgetPercent) < 80) bar.style.background = 'linear-gradient(90deg,#F59E0B,#FBBF24)'; else bar.style.background = 'linear-gradient(90deg,#EF4444,#DC2626)'; }
    const pctEl = document.getElementById('budget-pct-label'); if(pctEl) pctEl.textContent = `${budgetPercent}%`;
    const warning = document.getElementById('budget-warning'); if(warning) { if(parseFloat(budgetPercent) >= 80) { warning.textContent = `⚠️ ${budgetPercent}% budget use ho gaya!`; warning.style.display='block'; } else { warning.style.display='none'; } }
    // Budget left
    const budgetLeftEl = document.getElementById('hx-budget-left'); if(budgetLeftEl) budgetLeftEl.textContent = `₹${Math.max(0,Math.round(budgetLimit - totalExpense)).toLocaleString('en-IN')}`;
    // Today's stats for mini card
    const todayStr2 = new Date().toISOString().split('T')[0];
    const todayExps = familyExpenses.filter(e => e.date === todayStr2);
    const todayTotal = todayExps.reduce((s,e)=>s+parseFloat(e.amount),0);
    const todayEl = document.getElementById('hx-today-total'); if(todayEl) todayEl.textContent = `₹${Math.round(todayTotal).toLocaleString('en-IN')}`;
    const todayCountEl = document.getElementById('hx-today-count'); if(todayCountEl) todayCountEl.textContent = `${todayExps.length} transaction${todayExps.length !== 1 ? 's' : ''}`;
