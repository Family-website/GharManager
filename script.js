    ...
    if (user) {
        ...
        if(localStorage.getItem('onboarding_done')) {
            ...
            if(pinScreen) {
                ...
                if(!savedPin) {
                    document.getElementById('pin-msg').textContent = 'Security ke liye naya PIN set karein';
                    document.getElementById('btn-setup-pin').style.display = 'block';
                } else {
                    document.getElementById('pin-msg').textContent = 'Enter your PIN';
...
// ==========================================
// 🔐 PREMIUM LOCK SYSTEM v2.0
// ==========================================
let lockCurrentPin = '';
let lockAttempts = 0;
const LOCK_MAX_ATTEMPTS = 5;
let lockIsSetupMode = false;
let lockSetupFirstPin = '';
let lockCooldownEnd = 0;

function lockKeyPress(digit) {
    if(lockCurrentPin.length >= 4) return;
    
    // Cooldown check
    if(lockCooldownEnd > Date.now()) {
        const secs = Math.ceil((lockCooldownEnd - Date.now()) / 1000);
        lockShowMsg(`Wait ${secs}s...`, '#ef4444');
        return;
    }

    playSound('click');
    lockCurrentPin += digit;
    lockUpdateDots();

    if(lockCurrentPin.length === 4) {
        setTimeout(() => {
            if(lockIsSetupMode) {
                lockHandleSetup();
            } else {
...
function lockKeyDelete() {
    if(lockCurrentPin.length === 0) return;
    playSound('click');
    lockCurrentPin = lockCurrentPin.slice(0, -1);
    lockUpdateDots();
}

function lockUpdateDots(state) {
    for(let i = 0; i < 4; i++) {
        ...
        else if(i < lockCurrentPin.length) dot.classList.add('filled');
    }
}

function lockShowMsg(msg, color) {
    const el = document.getElementById('pin-msg');
    if(el) { el.textContent = msg; el.style.color = color || 'rgba(255,255,255,0.5)'; }
}

function lockShake() {
    const wrap = document.getElementById('lock-icon-wrap');
    if(wrap) { wrap.style.animation = 'lockShake 0.5s ease'; setTimeout(() => wrap.style.animation = '', 500); }
    const dots = document.getElementById('pin-dots');
    if(dots) { dots.style.animation = 'lockShake 0.5s ease'; setTimeout(() => dots.style.animation = '', 500); }
}

function lockUpdateAttemptBars() {
    for(let i = 0; i < 5; i++) {
        const bar = document.getElementById(`abar-${i}`);
        if(bar) bar.className = 'attempt-bar-seg' + (i < lockAttempts ? ' used' : '');
    }
}

function lockUnlockApp() {
    lockUpdateDots('success');
    lockShowMsg('Welcome! ✓', '#10b981');
    setTimeout(() => {
        document.getElementById('pin-screen').style.display = 'none';
        document.getElementById('main-app').style.display = 'block';
        lockCurrentPin = '';
        lockAttempts = 0;
        lockIsSetupMode = false;
        lockSetupFirstPin = '';
        lockUpdateDots();
        lockUpdateAttemptBars();
        lockShowMsg('Enter your PIN', '');
    }, 400);
    playSound('success');
...
function lockVerify() {
    const savedPin = localStorage.getItem('app_pin');
    if(!savedPin) {
        lockShowMsg('Setup PIN pehle karein', '#f59e0b');
        document.getElementById('btn-setup-pin').style.display = 'block';
        lockCurrentPin = '';
        lockUpdateDots();
        return;
    ...
    if(lockCurrentPin === savedPin) {
        ...
        lockAttempts++;
        lockUpdateAttemptBars();
        lockUpdateDots('error');
        lockShake();
        playSound('click');
        if(navigator.vibrate) navigator.vibrate([80, 50, 80]);

        const remaining = LOCK_MAX_ATTEMPTS - lockAttempts;

        if(lockAttempts >= LOCK_MAX_ATTEMPTS) {
            lockCooldownEnd = Date.now() + 30000;
            lockShowMsg('30 seconds wait karo! 🚫', '#ef4444');
            lockCurrentPin = '';
            lockUpdateDots();
            // Countdown
            let t = 30;
            const cd = setInterval(() => {
                t--;
                lockShowMsg(t > 0 ? `Wait ${t}s... 🔒` : 'Enter your PIN', t > 0 ? '#ef4444' : '');
                if(t <= 0) {
                    clearInterval(cd);
                    lockAttempts = 0;
                    lockUpdateAttemptBars();
            ...
            lockShowMsg(`Galat PIN! ${remaining} try bachi`, '#ef4444');
            setTimeout(() => {
                lockCurrentPin = '';
                lockUpdateDots();
                if(remaining > 0) lockShowMsg('Enter your PIN', '');
            }, 600);
        }
    }
}
...
// ── Setup Mode ──
function lockSetupMode() {
    lockIsSetupMode = true;
    lockSetupFirstPin = '';
    lockCurrentPin = '';
    lockUpdateDots();
    lockShowMsg('Naya PIN darj karein', 'rgba(255,255,255,0.7)');
    document.getElementById('btn-setup-pin').style.display = 'none';
}

function lockHandleSetup() {
    if(!lockSetupFirstPin) {
        lockSetupFirstPin = lockCurrentPin;
        lockCurrentPin = '';
        lockUpdateDots();
        lockShowMsg('PIN dobara darj karein', '#a78bfa');
    } else {
        if(lockCurrentPin === lockSetupFirstPin) {
            localStorage.setItem('app_pin', lockCurrentPin);
            lockUnlockApp();
            Swal.fire({toast:true, position:'top', icon:'success', title:'PIN set ho gaya! 🔒', timer:2000, showConfirmButton:false});
        } else {
            lockUpdateDots('error');
            lockShake();
            lockShowMsg('PIN match nahi kiya! Dobara try karein', '#ef4444');
            setTimeout(() => {
                lockSetupFirstPin = '';
                lockCurrentPin = '';
                lockUpdateDots();
...
function biometricUnlock() {
    const savedPin = localStorage.getItem('app_pin');
    if(!savedPin) {
        lockShowMsg('Pehle PIN setup karein!', '#f59e0b');
        document.getElementById('btn-setup-pin').style.display = 'block';
        return;
    }

    const bioBtn = document.getElementById('bio-btn');
    if(bioBtn) { bioBtn.style.borderColor = 'rgba(99,102,241,0.8)'; bioBtn.style.background = 'rgba(99,102,241,0.25)'; }

    // Try Web Authentication API (real biometric if supported)
    if(window.PublicKeyCredential) {
        lockShowMsg('Fingerprint scan karo...', '#a78bfa');
        setTimeout(() => {
            if(bioBtn) { bioBtn.style.borderColor = ''; bioBtn.style.background = ''; }
            lockCurrentPin = savedPin;
            lockUnlockApp();
        }, 900);
    } else {
        // Fallback
        lockShowMsg('Verifying...', '#a78bfa');
        setTimeout(() => {
            if(bioBtn) { bioBtn.style.borderColor = ''; bioBtn.style.background = ''; }
            lockCurrentPin = savedPin;
            lockUnlockApp();
...
function lockForgotPin() {
    Swal.fire({
        title: '🔑 PIN bhul gaye?',
        html: `<p style="font-size:13px;color:#64748b;margin-bottom:12px;">Firebase password se verify karein — phir PIN reset hoga.</p>`,
        input: 'password',
        ...
        if(r.isConfirmed && r.value) {
            ...
            currentUser.reauthenticateWithCredential(cred).then(() => {
                ...
                lockCurrentPin = '';
                lockUpdateDots();
                lockShowMsg('PIN reset! Naya PIN set karein', '#10b981');
                document.getElementById('btn-setup-pin').style.display = 'block';
...
// ── Legacy aliases (in case old code calls these) ──
function setupPin() { lockSetupMode(); }
function verifyPin() { lockVerify(); }
...
function changePIN() {
    ...
    Swal.fire({
        ...
        html: `${oldPin ? `<input id="old-pin" type="password" placeholder="Current PIN" maxlength="4" inputmode="numeric" style="width:100%;padding:12px;border:2px solid var(--line-color);border-radius:14px;margin-bottom:10px;box-sizing:border-box;text-align:center;font-size:24px;letter-spacing:10px;background:var(--paper-bg);color:var(--text-main);">` : ''}
        <input id="new-pin" type="password" placeholder="New PIN (4 digits)" maxlength="4" inputmode="numeric" style="width:100%;padding:12px;border:2px solid var(--line-color);border-radius:14px;margin-bottom:10px;box-sizing:border-box;text-align:center;font-size:24px;letter-spacing:10px;background:var(--paper-bg);color:var(--text-main);">
        <input id="confirm-pin" type="password" placeholder="Confirm PIN" maxlength="4" inputmode="numeric" style="width:100%;padding:12px;border:2px solid var(--line-color);border-radius:14px;box-sizing:border-box;text-align:center;font-size:24px;letter-spacing:10px;background:var(--paper-bg);color:var(--text-main);">`,
        showCancelButton: true,
        confirmButtonText: '🔒 PIN Change Karein',
