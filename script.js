// ==========================================
// 🔥 DATA CACHING (BUG FIX 1: INCOME PRESERVE)
// ==========================================
...
window.addEventListener('load', () => {
    setTimeout(() => {
        ...
        updateHisabUI(); // Initial Load Render
    }, 2500);
});
...
// ==========================================
// ☁️ CLOUD SYNC & LOCAL CACHE (Bug Fix 1)
// ==========================================
...
async function saveToCloud() { 
    // Always save local first (Solves Income Reset Bug)
...
// 🛡️ BUG FIX 5: BULLETPROOF EXPENSE ADD
function addExpense() {
    try {
        ...
        else if (typeof renderHistoryWithSkeleton === 'function') renderHistoryWithSkeleton();
...
function printReport() { 
    ...
    let win = window.open('','','width=800,height=600'); win.document.write(`<html><head><title>Print Report</title></head><body onload="window.print(); window.close();">${html}</body></html>`); win.document.close(); playSound('click'); 
}
...
// 🛡️ BUG FIX 2: SMART VOICE SAFETY
function startVoice() { 
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if(!SpeechRecognition) { Swal.fire('Oops!', 'Aapka browser Voice Typing support nahi karta.', 'warning'); return; }
    try {
        const recognition = new SpeechRecognition(); recognition.lang = 'hi-IN'; const btn = document.getElementById('mic-btn'); btn.innerText = "🛑"; 
        recognition.onresult = (event) => { 
            let text = event.results[0][0].transcript; let match = text.match(/\d+/); 
            if(match) { document.getElementById('amount').value = match[0]; let descStr = text.replace(match[0], '').replace(/(rupaye|rupay|roopaye|rs)/gi, '').trim(); document.getElementById('description').value = descStr; Swal.fire({ toast:true, position:'top-end', icon:'success', title:'✨ AI Smart Autofill!', timer: 2000, showConfirmButton: false }); } 
    ...
    } catch(e) { document.getElementById('mic-btn').innerText = "🎤"; console.error(e); Swal.fire('Error', 'Mic error.', 'error'); }
}
